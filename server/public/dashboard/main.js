import {
  ADMIN_ONLY_CARD_IDS,
  COLLAPSE_STORAGE_KEY,
  DASHBOARD_TZ_LABEL,
  DASHBOARD_TZ_OFFSET_HOURS,
  DEVICE_COLLAPSE_STORAGE_KEY,
  SESSION_STORAGE_KEY,
  createDashboardState
} from "./modules/constants.js";

const $ = (id) => document.getElementById(id);

const state = createDashboardState();

$("baseUrl").value = window.location.origin;
$("releaseExpiresAt").value = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();

    function pad2(value) {
      return String(value).padStart(2, "0");
    }

    function toOffsetDate(dateLike) {
      const ms = dateLike instanceof Date ? dateLike.getTime() : new Date(dateLike).getTime();
      if (Number.isNaN(ms)) {
        return null;
      }
      return new Date(ms + DASHBOARD_TZ_OFFSET_HOURS * 60 * 60 * 1000);
    }

    function formatIsoWithOffset(value) {
      if (!value || typeof value !== "string") {
        return "-";
      }
      const shifted = toOffsetDate(value);
      if (!shifted) {
        return value;
      }
      return `${shifted.getUTCFullYear()}-${pad2(shifted.getUTCMonth() + 1)}-${pad2(shifted.getUTCDate())} ${pad2(shifted.getUTCHours())}:${pad2(shifted.getUTCMinutes())}:${pad2(shifted.getUTCSeconds())} ${DASHBOARD_TZ_LABEL}`;
    }

    function nowTs() {
      const shifted = toOffsetDate(new Date());
      if (!shifted) {
        return new Date().toISOString().slice(11, 19);
      }
      return `${pad2(shifted.getUTCHours())}:${pad2(shifted.getUTCMinutes())}:${pad2(shifted.getUTCSeconds())}`;
    }

    function nowIso() {
      return new Date().toISOString();
    }

    function toDateTimeLocalValue(dateLike) {
      const date = dateLike instanceof Date ? dateLike : new Date(dateLike);
      if (Number.isNaN(date.getTime())) {
        return "";
      }
      return `${date.getFullYear()}-${pad2(date.getMonth() + 1)}-${pad2(date.getDate())}T${pad2(date.getHours())}:${pad2(date.getMinutes())}`;
    }

    function defaultScheduleOnceLocalValue() {
      const date = new Date(Date.now() + 5 * 60 * 1000);
      date.setSeconds(0, 0);
      return toDateTimeLocalValue(date);
    }

    function log(line) {
      const el = $("log");
      el.textContent += `[${nowTs()}] ${line}\n`;
      el.scrollTop = el.scrollHeight;
    }

    function baseUrl() {
      return ($("baseUrl").value || "").trim().replace(/\/+$/, "");
    }

    function realtimeUrl() {
      const base = baseUrl();
      if (!base) {
        return "";
      }

      try {
        const parsed = new URL(base);
        const wsProtocol = parsed.protocol === "https:" ? "wss:" : "ws:";
        return `${wsProtocol}//${parsed.host}/ws/client`;
      } catch {
        return "";
      }
    }

    function clearRealtimeReconnectTimer() {
      if (state.realtimeReconnectTimer) {
        clearTimeout(state.realtimeReconnectTimer);
        state.realtimeReconnectTimer = null;
      }
    }

    function clearRealtimeReloadTimer() {
      if (state.realtimeReloadTimer) {
        clearTimeout(state.realtimeReloadTimer);
        state.realtimeReloadTimer = null;
      }
    }

    function queueRealtimeReload() {
      if (state.realtimeReloadTimer) {
        return;
      }
      state.realtimeReloadTimer = setTimeout(async () => {
        state.realtimeReloadTimer = null;
        try {
          if (isAdminUser()) {
            await Promise.all([loadDevices(), loadOverview()]);
          } else {
            await loadDevices();
          }
        } catch (error) {
          log(`Realtime sync failed: ${error.message}`);
        }
      }, 800);
    }

    function withDeviceByUid(deviceUid, updater) {
      if (!deviceUid) {
        return false;
      }
      for (const device of state.devices) {
        if (!device || device.device_uid !== deviceUid) {
          continue;
        }
        updater(device);
        return true;
      }
      return false;
    }

    function ensureDeviceButtonStates(device) {
      if (!device || typeof device !== "object") {
        return {};
      }
      if (!device.button_states || typeof device.button_states !== "object" || Array.isArray(device.button_states)) {
        device.button_states = {};
      }
      const states = device.button_states;
      const buttonCount = Number.isInteger(device.button_count) ? device.button_count : 0;
      for (let i = 0; i < buttonCount; i += 1) {
        const key = String(i);
        if (!states[key] || typeof states[key] !== "object") {
          states[key] = {
            state: "unknown",
            ts: null,
            source: null
          };
        }
      }
      return states;
    }

    function inferButtonState(eventName, previousState = "unknown") {
      const normalized = String(eventName || "").trim().toLowerCase();
      if (
        normalized === "press" ||
        normalized === "pressed" ||
        normalized === "down" ||
        normalized === "hold" ||
        normalized === "long_press" ||
        normalized === "long-press" ||
        normalized === "repeat"
      ) {
        return "pressed";
      }

      if (
        normalized === "release" ||
        normalized === "released" ||
        normalized === "up"
      ) {
        return "released";
      }

      return previousState;
    }

    function applyButtonStateFromEvent(device, inputIndex, eventName, ts, source = "device") {
      if (!device || !Number.isInteger(inputIndex)) {
        return;
      }
      const buttonCount = Number.isInteger(device.button_count) ? device.button_count : 0;
      if (inputIndex < 0 || inputIndex >= buttonCount) {
        return;
      }

      const states = ensureDeviceButtonStates(device);
      const key = String(inputIndex);
      const existing = states[key] && typeof states[key] === "object" ? states[key] : {};
      const previousState =
        existing.state === "pressed" || existing.state === "released"
          ? existing.state
          : "unknown";
      const nextState = inferButtonState(eventName, previousState);

      states[String(inputIndex)] = {
        state: nextState,
        ts: typeof ts === "string" ? ts : nowIso(),
        source
      };
    }

    function initializeDeviceButtonStates(device) {
      ensureDeviceButtonStates(device);
      const inputEvent = device && device.last_input_event && typeof device.last_input_event === "object"
        ? device.last_input_event
        : null;
      if (!inputEvent) {
        return;
      }
      const details = inputEvent.details && typeof inputEvent.details === "object" ? inputEvent.details : {};
      const inputIndex = Number.isInteger(details.input_index) ? details.input_index : null;
      const eventName = typeof details.event === "string" ? details.event : null;
      if (inputIndex === null || !eventName) {
        return;
      }
      applyButtonStateFromEvent(device, inputIndex, eventName, inputEvent.created_at, inputEvent.source || "device");
    }

    function applyRealtimePresence(message, online) {
      const uid = typeof message.device_uid === "string" ? message.device_uid : "";
      const ts = typeof message.ts === "string" ? message.ts : nowIso();
      const updated = withDeviceByUid(uid, (device) => {
        device.ws_online = online;
        if (online) {
          device.last_seen_at = ts;
        }
      });
      if (updated) {
        renderDevices(state.devices);
        queueRealtimeReload();
      } else {
        queueRealtimeReload();
      }
    }

    function applyRealtimeState(message) {
      const uid = typeof message.device_uid === "string" ? message.device_uid : "";
      const relays = Array.isArray(message.relays)
        ? message.relays.map((value) => Boolean(value))
        : null;
      if (!uid || !relays) {
        return;
      }

      const ts = typeof message.ts === "string" ? message.ts : nowIso();
      const updated = withDeviceByUid(uid, (device) => {
        const previousRelays = Array.isArray(device.relays) ? device.relays : [];
        const nextRelays = [];
        for (let i = 0; i < relays.length; i += 1) {
          const existing = previousRelays.find((item) => Number(item.relay_index) === i) || {};
          nextRelays.push({
            relay_index: i,
            relay_name: typeof existing.relay_name === "string" ? existing.relay_name : `Relay ${i + 1}`,
            is_on: relays[i]
          });
        }
        device.relays = nextRelays;
        device.last_seen_at = ts;
        device.ws_online = true;
      });

      if (updated) {
        renderDevices(state.devices);
      } else {
        queueRealtimeReload();
      }
    }

    function applyRealtimeInputEvent(message) {
      const uid = typeof message.device_uid === "string" ? message.device_uid : "";
      if (!uid) {
        return;
      }

      const ts = typeof message.ts === "string" ? message.ts : nowIso();
      const details = {
        input_index: Number.isInteger(message.input_index) ? message.input_index : null,
        input_type: typeof message.input_type === "string" ? message.input_type : null,
        event: typeof message.event === "string" ? message.event : null,
        duration_ms: Number.isFinite(Number(message.duration_ms)) ? Number(message.duration_ms) : null,
        ts
      };

      const updated = withDeviceByUid(uid, (device) => {
        device.last_seen_at = ts;
        device.ws_online = true;
        device.last_input_event = {
          source: "device",
          created_at: ts,
          details
        };
        if (Number.isInteger(details.input_index) && typeof details.event === "string") {
          applyButtonStateFromEvent(device, details.input_index, details.event, ts, "realtime");
        }
      });

      const eventName = typeof details.event === "string" ? details.event : "event";
      const inputIndex = Number.isInteger(details.input_index) ? `B${details.input_index}` : "B?";
      const durationText =
        Number.isFinite(details.duration_ms) && details.duration_ms > 0 ? ` (${details.duration_ms}ms)` : "";
      log(`Input event: ${uid} ${inputIndex} ${eventName}${durationText}`);

      if (updated) {
        renderDevices(state.devices);
      } else {
        queueRealtimeReload();
      }
    }

    function sendRealtimeAuth() {
      const socket = state.realtimeSocket;
      if (!socket || socket.readyState !== WebSocket.OPEN || !state.accessToken) {
        return;
      }
      socket.send(
        JSON.stringify({
          type: "auth",
          access_token: state.accessToken
        })
      );
    }

    function scheduleRealtimeReconnect(reason) {
      if (!state.user || !state.accessToken || state.realtimeReconnectTimer) {
        return;
      }
      const attempt = Math.min(state.realtimeReconnectAttempt, 5);
      const delayMs = Math.min(15000, 1000 * Math.pow(2, attempt));
      state.realtimeReconnectAttempt += 1;
      log(`Realtime disconnected (${reason}). Reconnecting in ${Math.round(delayMs / 1000)}s.`);
      state.realtimeReconnectTimer = setTimeout(() => {
        state.realtimeReconnectTimer = null;
        connectRealtime();
      }, delayMs);
    }

    function stopRealtime(options = {}) {
      const { silent = false } = options;
      clearRealtimeReconnectTimer();
      clearRealtimeReloadTimer();
      state.realtimeReconnectAttempt = 0;
      state.realtimeAuthed = false;
      rejectPendingRealtimeCommands("channel_stopped");

      const socket = state.realtimeSocket;
      state.realtimeSocket = null;
      state.realtimeUrl = "";

      if (socket && (socket.readyState === WebSocket.OPEN || socket.readyState === WebSocket.CONNECTING)) {
        socket.close(1000, "client_stop");
      }

      if (!silent) {
        log("Realtime channel stopped.");
      }
    }

    async function handleRealtimeMessage(socket, rawData) {
      let text = "";
      if (typeof rawData === "string") {
        text = rawData;
      } else if (rawData instanceof Blob) {
        text = await rawData.text();
      } else {
        return;
      }

      const message = safeParseJson(text, null);
      if (!message || typeof message !== "object") {
        return;
      }

      const type = typeof message.type === "string" ? message.type : "";
      if (!type) {
        return;
      }

      if (socket !== state.realtimeSocket) {
        return;
      }

      if (type === "auth_ok") {
        state.realtimeAuthed = true;
        state.realtimeReconnectAttempt = 0;
        log("Realtime auth successful.");
        return;
      }

      if (type === "auth_error") {
        state.realtimeAuthed = false;
        const code = typeof message.code === "string" ? message.code : "auth_error";
        log(`Realtime auth failed: ${code}`);
        if (code === "invalid_token" && state.refreshToken) {
          try {
            await refreshAccessToken();
            sendRealtimeAuth();
          } catch {
            scheduleRealtimeReconnect("auth_failed");
          }
        }
        return;
      }

      if (!state.realtimeAuthed) {
        return;
      }

      if (type === "device_state") {
        applyRealtimeState(message);
        return;
      }

      if (type === "input_event") {
        applyRealtimeInputEvent(message);
        return;
      }

      if (type === "device_online") {
        applyRealtimePresence(message, true);
        const uid = typeof message.device_uid === "string" ? message.device_uid : "device";
        log(`Device online: ${uid}`);
        return;
      }

      if (type === "device_offline") {
        applyRealtimePresence(message, false);
        const uid = typeof message.device_uid === "string" ? message.device_uid : "device";
        log(`Device offline: ${uid}`);
        return;
      }

      if (type === "ota_status") {
        const uid = typeof message.device_uid === "string" ? message.device_uid : "device";
        const eventType = typeof message.event_type === "string" ? message.event_type : "status";
        const status = typeof message.status === "string" ? message.status : "unknown";
        const reason = typeof message.reason === "string" && message.reason ? ` reason=${message.reason}` : "";
        const fromVersion = typeof message.from_version === "string" && message.from_version
          ? ` from=${message.from_version}`
          : "";
        const toVersion = typeof message.to_version === "string" && message.to_version
          ? ` to=${message.to_version}`
          : "";
        const securityVersion =
          typeof message.security_version === "number"
            ? ` security=${message.security_version}`
            : "";
        const commandId = typeof message.command_id === "string" && message.command_id
          ? ` cmd=${message.command_id}`
          : "";
        log(`OTA status: ${uid} ${eventType} ${status}${reason}${fromVersion}${toVersion}${securityVersion}${commandId}`);
        return;
      }

      if (type === "cmd_ack") {
        const consumed = resolveRealtimeCommandAck(message);
        if (!consumed && message.ok === false) {
          const code = typeof message.code === "string" ? message.code : "command_failed";
          const detail = typeof message.message === "string" ? message.message : "";
          log(`Realtime command failed: ${code}${detail ? ` (${detail})` : ""}`);
        }
        return;
      }
    }

    function connectRealtime(forceReconnect = false) {
      if (!state.user || !state.accessToken) {
        return;
      }

      const endpoint = realtimeUrl();
      if (!endpoint) {
        return;
      }

      const current = state.realtimeSocket;
      if (current) {
        const isSameUrl = state.realtimeUrl === endpoint;
        if (
          !forceReconnect &&
          isSameUrl &&
          (current.readyState === WebSocket.OPEN || current.readyState === WebSocket.CONNECTING)
        ) {
          return;
        }
        stopRealtime({ silent: true });
      }

      clearRealtimeReconnectTimer();
      state.realtimeUrl = endpoint;

      const socket = new WebSocket(endpoint);
      state.realtimeSocket = socket;
      state.realtimeAuthed = false;

      socket.addEventListener("open", () => {
        if (socket !== state.realtimeSocket) {
          return;
        }
        state.realtimeReconnectAttempt = 0;
        log("Realtime channel connected.");
        sendRealtimeAuth();
      });

      socket.addEventListener("message", (event) => {
        void handleRealtimeMessage(socket, event.data);
      });

      socket.addEventListener("close", (event) => {
        if (socket !== state.realtimeSocket) {
          return;
        }
        state.realtimeSocket = null;
        state.realtimeAuthed = false;
        rejectPendingRealtimeCommands(`close_${event.code}`);
        if (state.user && state.accessToken) {
          scheduleRealtimeReconnect(`code ${event.code}`);
        }
      });

      socket.addEventListener("error", () => {
        if (socket !== state.realtimeSocket) {
          return;
        }
        log("Realtime channel error.");
      });
    }

    function safeParseJson(text, fallback) {
      if (!text) {
        return fallback;
      }
      try {
        return JSON.parse(text);
      } catch {
        return fallback;
      }
    }

    function escapeHtml(value) {
      return String(value ?? "")
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        .replace(/"/g, "&quot;")
        .replace(/'/g, "&#39;");
    }

    async function withDeviceActionLock(deviceId, fn) {
      const key = String(deviceId || "").trim();
      if (!key) {
        return fn();
      }

      const previous = state.pendingDeviceActions.get(key) || Promise.resolve();
      const run = previous
        .catch(() => undefined)
        .then(() => fn());

      state.pendingDeviceActions.set(
        key,
        run.finally(() => {
          if (state.pendingDeviceActions.get(key) === run) {
            state.pendingDeviceActions.delete(key);
          }
        })
      );

      return run;
    }

    function getAutomationTestingState(deviceId) {
      const key = String(deviceId || "").trim();
      if (!key) {
        return {
          loading: false,
          error: "",
          loadedAt: "",
          automations: [],
          schedules: []
        };
      }

      const cached = state.automationTestingByDevice.get(key);
      if (!cached || typeof cached !== "object") {
        return {
          loading: false,
          error: "",
          loadedAt: "",
          automations: [],
          schedules: []
        };
      }
      return {
        loading: cached.loading === true,
        error: typeof cached.error === "string" ? cached.error : "",
        loadedAt: typeof cached.loadedAt === "string" ? cached.loadedAt : "",
        automations: Array.isArray(cached.automations) ? cached.automations : [],
        schedules: Array.isArray(cached.schedules) ? cached.schedules : []
      };
    }

    function setAutomationTestingState(deviceId, next) {
      const key = String(deviceId || "").trim();
      if (!key) {
        return;
      }
      const current = getAutomationTestingState(key);
      state.automationTestingByDevice.set(key, {
        ...current,
        ...next
      });
    }

    async function loadAutomationTesting(deviceId, options = {}) {
      const key = String(deviceId || "").trim();
      if (!key) {
        return {
          automations: [],
          schedules: []
        };
      }

      const device = (state.devices || []).find((entry) => entry && entry.id === key);
      if (!device || !device.owner_user_id) {
        setAutomationTestingState(key, {
          loading: false,
          error: device ? "Device is unclaimed." : "Device not found.",
          loadedAt: "",
          automations: [],
          schedules: []
        });
        renderDevices(state.devices);
        return {
          automations: [],
          schedules: []
        };
      }

      const force = options && options.force === true;
      const current = getAutomationTestingState(key);
      if (!force && current.loadedAt && !current.error) {
        return {
          automations: current.automations,
          schedules: current.schedules
        };
      }

      setAutomationTestingState(key, {
        loading: true,
        error: ""
      });
      renderDevices(state.devices);

      try {
        const [automations, schedules] = await Promise.all([
          api(`/api/v1/admin/devices/${encodeURIComponent(key)}/automations`, { method: "GET" }),
          api(`/api/v1/admin/devices/${encodeURIComponent(key)}/schedules`, { method: "GET" })
        ]);
        const normalizedAutomations = Array.isArray(automations) ? automations : [];
        const normalizedSchedules = Array.isArray(schedules) ? schedules : [];
        setAutomationTestingState(key, {
          loading: false,
          error: "",
          loadedAt: nowIso(),
          automations: normalizedAutomations,
          schedules: normalizedSchedules
        });
        renderDevices(state.devices);
        return {
          automations: normalizedAutomations,
          schedules: normalizedSchedules
        };
      } catch (error) {
        const message = error instanceof Error ? error.message : "load_failed";
        setAutomationTestingState(key, {
          loading: false,
          error: message
        });
        renderDevices(state.devices);
        throw error;
      }
    }

    function syncAutomationTestingControls(deviceId) {
      const key = String(deviceId || "").trim();
      if (!key) {
        return;
      }

      const triggerTypeEl = document.querySelector(`[data-automation-trigger-type="${key}"]`);
      const inputIndexEl = document.querySelector(`[data-automation-input-index="${key}"]`);
      const eventEl = document.querySelector(`[data-automation-event="${key}"]`);
      const holdSecondsEl = document.querySelector(`[data-automation-hold-seconds="${key}"]`);
      const actionTypeEl = document.querySelector(`[data-automation-action-type="${key}"]`);
      const relayIndexEl = document.querySelector(`[data-automation-relay-index="${key}"]`);
      const actionEl = document.querySelector(`[data-automation-action="${key}"]`);
      const scheduleTypeEl = document.querySelector(`[data-schedule-type="${key}"]`);
      const scheduleCronEl = document.querySelector(`[data-schedule-cron="${key}"]`);
      const scheduleOnceAtEl = document.querySelector(`[data-schedule-once-at="${key}"]`);
      const scheduleScopeEl = document.querySelector(`[data-schedule-target-scope="${key}"]`);
      const scheduleRelayIndexEl = document.querySelector(`[data-schedule-relay-index="${key}"]`);
      const scheduleActionEl = document.querySelector(`[data-schedule-action="${key}"]`);

      if (
        triggerTypeEl instanceof HTMLSelectElement &&
        inputIndexEl instanceof HTMLSelectElement &&
        eventEl instanceof HTMLSelectElement &&
        holdSecondsEl instanceof HTMLInputElement
      ) {
        const triggerType = triggerTypeEl.value;
        const inputBased = triggerType === "input_event" || triggerType === "button_hold";
        inputIndexEl.disabled = !inputBased;
        eventEl.disabled = !inputBased;
        holdSecondsEl.disabled = triggerType !== "button_hold";
        if (triggerType === "button_hold") {
          eventEl.value = "hold";
        }
      }

      if (
        actionTypeEl instanceof HTMLSelectElement &&
        relayIndexEl instanceof HTMLSelectElement &&
        actionEl instanceof HTMLSelectElement
      ) {
        const actionType = actionTypeEl.value;
        relayIndexEl.disabled = actionType !== "set_relay";
        const toggleOption = Array.from(actionEl.options).find((option) => option.value === "toggle");
        if (toggleOption) {
          toggleOption.disabled = actionType === "set_all_relays";
        }
        if (actionType === "set_all_relays" && actionEl.value === "toggle") {
          actionEl.value = "off";
        }
      }

      if (
        scheduleTypeEl instanceof HTMLSelectElement &&
        scheduleCronEl instanceof HTMLInputElement &&
        scheduleOnceAtEl instanceof HTMLInputElement
      ) {
        const scheduleType = scheduleTypeEl.value;
        scheduleCronEl.disabled = scheduleType !== "cron";
        scheduleOnceAtEl.disabled = scheduleType !== "once";
      }

      if (
        scheduleScopeEl instanceof HTMLSelectElement &&
        scheduleRelayIndexEl instanceof HTMLSelectElement &&
        scheduleActionEl instanceof HTMLSelectElement
      ) {
        const scope = scheduleScopeEl.value;
        scheduleRelayIndexEl.disabled = scope !== "single";
        const toggleOption = Array.from(scheduleActionEl.options).find((option) => option.value === "toggle");
        if (toggleOption) {
          toggleOption.disabled = scope !== "single";
        }
        if (scope !== "single" && scheduleActionEl.value === "toggle") {
          scheduleActionEl.value = "off";
        }
      }
    }

    function collectAutomationCreatePayload(device, card) {
      const deviceId = String(device && device.id ? device.id : "");
      const relayCount = Number.isInteger(device?.relay_count) ? device.relay_count : 0;
      const buttonCount = Number.isInteger(device?.button_count) ? device.button_count : 0;

      const nameEl = card.querySelector(`[data-automation-name="${deviceId}"]`);
      const triggerTypeEl = card.querySelector(`[data-automation-trigger-type="${deviceId}"]`);
      const inputIndexEl = card.querySelector(`[data-automation-input-index="${deviceId}"]`);
      const eventEl = card.querySelector(`[data-automation-event="${deviceId}"]`);
      const holdSecondsEl = card.querySelector(`[data-automation-hold-seconds="${deviceId}"]`);
      const actionTypeEl = card.querySelector(`[data-automation-action-type="${deviceId}"]`);
      const relayIndexEl = card.querySelector(`[data-automation-relay-index="${deviceId}"]`);
      const actionEl = card.querySelector(`[data-automation-action="${deviceId}"]`);
      const cooldownEl = card.querySelector(`[data-automation-cooldown="${deviceId}"]`);
      const conditionRelayEl = card.querySelector(`[data-automation-condition-relay="${deviceId}"]`);
      const conditionStateEl = card.querySelector(`[data-automation-condition-state="${deviceId}"]`);
      const enabledEl = card.querySelector(`[data-automation-enabled="${deviceId}"]`);

      if (!(nameEl instanceof HTMLInputElement)) throw new Error("Automation name field is missing.");
      if (!(triggerTypeEl instanceof HTMLSelectElement)) throw new Error("Automation trigger field is missing.");
      if (!(inputIndexEl instanceof HTMLSelectElement)) throw new Error("Automation input index field is missing.");
      if (!(eventEl instanceof HTMLSelectElement)) throw new Error("Automation event field is missing.");
      if (!(holdSecondsEl instanceof HTMLInputElement)) throw new Error("Automation hold field is missing.");
      if (!(actionTypeEl instanceof HTMLSelectElement)) throw new Error("Automation action type field is missing.");
      if (!(relayIndexEl instanceof HTMLSelectElement)) throw new Error("Automation relay index field is missing.");
      if (!(actionEl instanceof HTMLSelectElement)) throw new Error("Automation action field is missing.");
      if (!(cooldownEl instanceof HTMLInputElement)) throw new Error("Automation cooldown field is missing.");
      if (!(conditionRelayEl instanceof HTMLSelectElement)) throw new Error("Automation condition relay field is missing.");
      if (!(conditionStateEl instanceof HTMLSelectElement)) throw new Error("Automation condition state field is missing.");
      if (!(enabledEl instanceof HTMLInputElement)) throw new Error("Automation enabled field is missing.");

      const name = nameEl.value.trim();
      if (!name) {
        throw new Error("Automation name is required.");
      }

      const triggerType = triggerTypeEl.value;
      const triggerConfig = {};

      if (triggerType === "input_event" || triggerType === "button_hold") {
        const inputIndex = Number.parseInt(inputIndexEl.value, 10);
        if (!Number.isInteger(inputIndex) || inputIndex < 0 || inputIndex >= buttonCount) {
          throw new Error("Automation input index is invalid.");
        }
        triggerConfig.input_index = inputIndex;
        const event = eventEl.value.trim();
        if (event) {
          triggerConfig.event = event;
        }
      }

      if (triggerType === "button_hold") {
        const holdSeconds = Number.parseInt(holdSecondsEl.value, 10);
        if (!Number.isInteger(holdSeconds) || holdSeconds < 1 || holdSeconds > 600) {
          throw new Error("Hold seconds must be between 1 and 600.");
        }
        triggerConfig.hold_seconds = holdSeconds;
        triggerConfig.event = "hold";
      }

      const actionType = actionTypeEl.value;
      const action = actionEl.value;
      const actionConfig = { action };

      if (actionType === "set_relay") {
        const relayIndex = Number.parseInt(relayIndexEl.value, 10);
        if (!Number.isInteger(relayIndex) || relayIndex < 0 || relayIndex >= relayCount) {
          throw new Error("Automation relay index is invalid.");
        }
        actionConfig.relay_index = relayIndex;
      } else if (action === "toggle") {
        throw new Error("set_all_relays supports on/off only.");
      }

      const cooldownSeconds = Number.parseInt(cooldownEl.value || "0", 10);
      if (!Number.isInteger(cooldownSeconds) || cooldownSeconds < 0 || cooldownSeconds > 86400) {
        throw new Error("Cooldown must be between 0 and 86400 seconds.");
      }

      const conditionConfig = {};
      const conditionRelay = Number.parseInt(conditionRelayEl.value, 10);
      if (Number.isInteger(conditionRelay) && conditionRelay >= 0 && conditionRelay < relayCount) {
        const stateValue = conditionStateEl.value === "on";
        conditionConfig.required_relay_state = {
          relay_index: conditionRelay,
          is_on: stateValue
        };
      }

      return {
        name,
        trigger_type: triggerType,
        trigger_config: triggerConfig,
        condition_config: conditionConfig,
        action_type: actionType,
        action_config: actionConfig,
        cooldown_seconds: cooldownSeconds,
        is_enabled: enabledEl.checked
      };
    }

    function collectScheduleCreatePayload(device, card) {
      const deviceId = String(device && device.id ? device.id : "");
      const relayCount = Number.isInteger(device?.relay_count) ? device.relay_count : 0;

      const nameEl = card.querySelector(`[data-schedule-name="${deviceId}"]`);
      const typeEl = card.querySelector(`[data-schedule-type="${deviceId}"]`);
      const cronEl = card.querySelector(`[data-schedule-cron="${deviceId}"]`);
      const onceAtEl = card.querySelector(`[data-schedule-once-at="${deviceId}"]`);
      const timezoneEl = card.querySelector(`[data-schedule-timezone="${deviceId}"]`);
      const scopeEl = card.querySelector(`[data-schedule-target-scope="${deviceId}"]`);
      const relayIndexEl = card.querySelector(`[data-schedule-relay-index="${deviceId}"]`);
      const actionEl = card.querySelector(`[data-schedule-action="${deviceId}"]`);
      const enabledEl = card.querySelector(`[data-schedule-enabled="${deviceId}"]`);

      if (!(nameEl instanceof HTMLInputElement)) throw new Error("Schedule name field is missing.");
      if (!(typeEl instanceof HTMLSelectElement)) throw new Error("Schedule type field is missing.");
      if (!(cronEl instanceof HTMLInputElement)) throw new Error("Schedule cron field is missing.");
      if (!(onceAtEl instanceof HTMLInputElement)) throw new Error("Schedule time field is missing.");
      if (!(timezoneEl instanceof HTMLInputElement)) throw new Error("Schedule timezone field is missing.");
      if (!(scopeEl instanceof HTMLSelectElement)) throw new Error("Schedule scope field is missing.");
      if (!(relayIndexEl instanceof HTMLSelectElement)) throw new Error("Schedule relay index field is missing.");
      if (!(actionEl instanceof HTMLSelectElement)) throw new Error("Schedule action field is missing.");
      if (!(enabledEl instanceof HTMLInputElement)) throw new Error("Schedule enabled field is missing.");

      const scheduleType = typeEl.value;
      const targetScope = scopeEl.value;
      const action = actionEl.value;
      const timezone = timezoneEl.value.trim() || "UTC";

      if (targetScope === "all" && action === "toggle") {
        throw new Error("All-relays schedules support on/off only.");
      }

      const payload = {
        target_scope: targetScope,
        relay_index: null,
        name: nameEl.value.trim() || null,
        schedule_type: scheduleType,
        cron_expression: null,
        execute_at: null,
        timezone,
        action,
        is_enabled: enabledEl.checked
      };

      if (targetScope === "single") {
        const relayIndex = Number.parseInt(relayIndexEl.value, 10);
        if (!Number.isInteger(relayIndex) || relayIndex < 0 || relayIndex >= relayCount) {
          throw new Error("Schedule relay index is invalid.");
        }
        payload.relay_index = relayIndex;
      }

      if (scheduleType === "cron") {
        const cronExpression = cronEl.value.trim();
        if (!cronExpression) {
          throw new Error("Cron expression is required for cron schedules.");
        }
        payload.cron_expression = cronExpression;
      } else {
        const onceLocal = onceAtEl.value.trim();
        if (!onceLocal) {
          throw new Error("Execution time is required for once schedules.");
        }
        const parsed = new Date(onceLocal);
        if (Number.isNaN(parsed.getTime())) {
          throw new Error("Execution time is invalid.");
        }
        payload.execute_at = parsed.toISOString();
      }

      return payload;
    }

    function renderAutomationTesting(device) {
      if (!isAdminUser()) {
        return "";
      }
      const deviceId = String(device && device.id ? device.id : "");
      if (!deviceId) {
        return "";
      }

      const claimed = Boolean(device.owner_user_id);
      const testing = getAutomationTestingState(deviceId);
      const loadLabel = testing.loading ? "Loading..." : "Load Rules";
      const loadDisabled = claimed ? (testing.loading ? "disabled" : "") : "disabled";
      const loadedAtText = testing.loadedAt ? formatIsoWithOffset(testing.loadedAt) : "-";
      const relayNames = Array.isArray(device?.relay_names) ? device.relay_names : [];
      const relayCount = Number.isInteger(device?.relay_count) ? device.relay_count : 0;
      const buttonCount = Number.isInteger(device?.button_count) ? device.button_count : 0;
      const relayOptions = relayCount > 0
        ? Array.from({ length: relayCount }, (_, relayIndex) => {
            const relayName = typeof relayNames[relayIndex] === "string"
              ? relayNames[relayIndex]
              : `Relay ${relayIndex + 1}`;
            return `<option value="${relayIndex}">R${relayIndex} - ${escapeHtml(relayName)}</option>`;
          }).join("")
        : '<option value="">No relays</option>';
      const relayConditionOptions = relayCount > 0
        ? `<option value="">none</option>${Array.from({ length: relayCount }, (_, relayIndex) => {
            const relayName = typeof relayNames[relayIndex] === "string"
              ? relayNames[relayIndex]
              : `Relay ${relayIndex + 1}`;
            return `<option value="${relayIndex}">R${relayIndex} - ${escapeHtml(relayName)}</option>`;
          }).join("")}`
        : '<option value="">none</option>';
      const buttonOptions = buttonCount > 0
        ? Array.from({ length: buttonCount }, (_, inputIndex) => `<option value="${inputIndex}">B${inputIndex}</option>`).join("")
        : '<option value="">No buttons</option>';
      const onceLocalDefault = defaultScheduleOnceLocalValue();

      const automationRows = testing.automations.length > 0
        ? testing.automations.map((rule) => {
            const id = String(rule && rule.id ? rule.id : "");
            const name = String(rule && rule.name ? rule.name : "Unnamed automation");
            const triggerType = String(rule && rule.trigger_type ? rule.trigger_type : "-");
            const actionType = String(rule && rule.action_type ? rule.action_type : "-");
            const enabled = rule && rule.is_enabled === false ? "disabled" : "enabled";
            return `
              <div class="automation-test-row">
                <div class="automation-test-row-head">
                  <div>
                    <strong>${escapeHtml(name)}</strong>
                    <div class="automation-test-row-meta code">${escapeHtml(id)}</div>
                    <div class="automation-test-row-meta">${escapeHtml(triggerType)} -> ${escapeHtml(actionType)} (${escapeHtml(enabled)})</div>
                  </div>
                  <div class="row">
                    <button data-run-automation-now="${escapeHtml(deviceId)}:${escapeHtml(id)}" ${claimed ? "" : "disabled"}>Run Automation Now</button>
                    <button class="danger" data-delete-automation="${escapeHtml(deviceId)}:${escapeHtml(id)}" ${claimed ? "" : "disabled"}>Delete</button>
                  </div>
                </div>
              </div>
            `;
          }).join("")
        : '<div class="muted">No automations for this device owner.</div>';

      const scheduleRows = testing.schedules.length > 0
        ? testing.schedules.map((schedule) => {
            const id = String(schedule && schedule.id ? schedule.id : "");
            const name = String(schedule && schedule.name ? schedule.name : "Unnamed schedule");
            const scheduleType = String(schedule && schedule.schedule_type ? schedule.schedule_type : "-");
            const action = String(schedule && schedule.action ? schedule.action : "-");
            const enabled = schedule && schedule.is_enabled === false ? "disabled" : "enabled";
            return `
              <div class="automation-test-row">
                <div class="automation-test-row-head">
                  <div>
                    <strong>${escapeHtml(name)}</strong>
                    <div class="automation-test-row-meta code">${escapeHtml(id)}</div>
                    <div class="automation-test-row-meta">${escapeHtml(scheduleType)} -> ${escapeHtml(action)} (${escapeHtml(enabled)})</div>
                  </div>
                  <button data-run-schedule-now="${escapeHtml(deviceId)}:${escapeHtml(id)}" ${claimed ? "" : "disabled"}>Run Schedule Now</button>
                </div>
              </div>
            `;
          }).join("")
        : '<div class="muted">No schedules for this device owner.</div>';

      return `
        <section class="device-block">
          <div class="device-block-title">Automation Testing (Admin)</div>
          <div class="automation-test-stack">
            <div class="device-action-row">
              <button data-load-automation-tests="${escapeHtml(deviceId)}" class="secondary" ${loadDisabled}>${escapeHtml(loadLabel)}</button>
            </div>
            <div class="meta">claimed: ${claimed ? "yes" : "no"} | loaded_at: ${escapeHtml(loadedAtText)}</div>
            ${testing.error ? `<div class="meta" style="color: var(--danger);">${escapeHtml(testing.error)}</div>` : ""}
            ${!claimed ? '<div class="muted">Run-now controls are available only for claimed devices.</div>' : ""}
            <div class="automation-create-grid">
              <section class="automation-create-section">
                <div class="meta"><strong>Create Automation</strong></div>
                <div>
                  <label>Name</label>
                  <input data-automation-name="${escapeHtml(deviceId)}" placeholder="Example: Hold B0 turns all off" />
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Trigger</label>
                    <select data-automation-trigger-type="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      <option value="input_event">input_event</option>
                      <option value="button_hold">button_hold</option>
                      <option value="device_online">device_online</option>
                      <option value="device_offline">device_offline</option>
                    </select>
                  </div>
                  <div>
                    <label>Input Index</label>
                    <select data-automation-input-index="${escapeHtml(deviceId)}" ${claimed && buttonCount > 0 ? "" : "disabled"}>
                      ${buttonOptions}
                    </select>
                  </div>
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Event</label>
                    <select data-automation-event="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      <option value="press">press</option>
                      <option value="release">release</option>
                      <option value="hold">hold</option>
                    </select>
                  </div>
                  <div>
                    <label>Hold Seconds (button_hold)</label>
                    <input data-automation-hold-seconds="${escapeHtml(deviceId)}" type="number" min="1" max="600" value="2" ${claimed ? "" : "disabled"} />
                  </div>
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Action Type</label>
                    <select data-automation-action-type="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      <option value="set_relay">set_relay</option>
                      <option value="set_all_relays">set_all_relays</option>
                    </select>
                  </div>
                  <div>
                    <label>Relay Index (set_relay)</label>
                    <select data-automation-relay-index="${escapeHtml(deviceId)}" ${claimed && relayCount > 0 ? "" : "disabled"}>
                      ${relayOptions}
                    </select>
                  </div>
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Action</label>
                    <select data-automation-action="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      <option value="on">on</option>
                      <option value="off">off</option>
                      <option value="toggle">toggle</option>
                    </select>
                  </div>
                  <div>
                    <label>Cooldown Seconds</label>
                    <input data-automation-cooldown="${escapeHtml(deviceId)}" type="number" min="0" max="86400" value="0" ${claimed ? "" : "disabled"} />
                  </div>
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Condition Relay (optional)</label>
                    <select data-automation-condition-relay="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      ${relayConditionOptions}
                    </select>
                  </div>
                  <div>
                    <label>Condition State</label>
                    <select data-automation-condition-state="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      <option value="on">on</option>
                      <option value="off">off</option>
                    </select>
                  </div>
                </div>
                <label class="io-inline-check">
                  <input type="checkbox" data-automation-enabled="${escapeHtml(deviceId)}" checked ${claimed ? "" : "disabled"} />
                  Enabled
                </label>
                <div class="device-action-row">
                  <button data-create-automation="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>Create Automation</button>
                </div>
              </section>

              <section class="automation-create-section">
                <div class="meta"><strong>Create Time Schedule</strong></div>
                <div>
                  <label>Name (optional)</label>
                  <input data-schedule-name="${escapeHtml(deviceId)}" placeholder="Example: Night OFF" />
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Schedule Type</label>
                    <select data-schedule-type="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      <option value="once">once</option>
                      <option value="cron">cron</option>
                    </select>
                  </div>
                  <div>
                    <label>Timezone</label>
                    <input data-schedule-timezone="${escapeHtml(deviceId)}" value="UTC" ${claimed ? "" : "disabled"} />
                  </div>
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Once At</label>
                    <input data-schedule-once-at="${escapeHtml(deviceId)}" type="datetime-local" value="${escapeHtml(onceLocalDefault)}" ${claimed ? "" : "disabled"} />
                  </div>
                  <div>
                    <label>Cron Expression</label>
                    <input data-schedule-cron="${escapeHtml(deviceId)}" value="*/5 * * * *" ${claimed ? "" : "disabled"} />
                  </div>
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Target Scope</label>
                    <select data-schedule-target-scope="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      <option value="single">single</option>
                      <option value="all">all</option>
                    </select>
                  </div>
                  <div>
                    <label>Relay Index (single)</label>
                    <select data-schedule-relay-index="${escapeHtml(deviceId)}" ${claimed && relayCount > 0 ? "" : "disabled"}>
                      ${relayOptions}
                    </select>
                  </div>
                </div>
                <div class="grid cols-2">
                  <div>
                    <label>Action</label>
                    <select data-schedule-action="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>
                      <option value="on">on</option>
                      <option value="off">off</option>
                      <option value="toggle">toggle</option>
                    </select>
                  </div>
                  <div>
                    <label class="io-inline-check" style="margin-top: 22px;">
                      <input type="checkbox" data-schedule-enabled="${escapeHtml(deviceId)}" checked ${claimed ? "" : "disabled"} />
                      Enabled
                    </label>
                  </div>
                </div>
                <div class="device-action-row">
                  <button data-create-schedule="${escapeHtml(deviceId)}" ${claimed ? "" : "disabled"}>Create Schedule</button>
                </div>
              </section>
            </div>
            <div>
              <div class="meta"><strong>Automations</strong> (${testing.automations.length})</div>
              <div class="automation-test-list">${automationRows}</div>
            </div>
            <div>
              <div class="meta"><strong>Schedules</strong> (${testing.schedules.length})</div>
              <div class="automation-test-list">${scheduleRows}</div>
            </div>
          </div>
        </section>
      `;
    }

    function parseIsoMs(value) {
      if (typeof value !== "string" || !value.trim()) {
        return null;
      }
      const ms = new Date(value).getTime();
      return Number.isNaN(ms) ? null : ms;
    }

    function ageLabel(iso) {
      const ms = parseIsoMs(iso);
      if (ms === null) {
        return "-";
      }
      const ageSec = Math.max(0, Math.floor((Date.now() - ms) / 1000));
      if (ageSec < 60) {
        return `${ageSec}s ago`;
      }
      const ageMin = Math.floor(ageSec / 60);
      if (ageMin < 60) {
        return `${ageMin}m ago`;
      }
      const ageHr = Math.floor(ageMin / 60);
      if (ageHr < 24) {
        return `${ageHr}h ago`;
      }
      return `${Math.floor(ageHr / 24)}d ago`;
    }

    function isLikelyOnline(lastSeenAt) {
      const ms = parseIsoMs(lastSeenAt);
      if (ms === null) {
        return false;
      }
      return Date.now() - ms <= 90_000;
    }

    function persistCollapsedCards() {
      localStorage.setItem(COLLAPSE_STORAGE_KEY, JSON.stringify(state.collapsedCards || {}));
    }

    function loadCollapsedCards() {
      const stored = safeParseJson(localStorage.getItem(COLLAPSE_STORAGE_KEY), {});
      if (stored && typeof stored === "object" && !Array.isArray(stored)) {
        state.collapsedCards = stored;
      }
    }

    function persistCollapsedDeviceCards() {
      localStorage.setItem(
        DEVICE_COLLAPSE_STORAGE_KEY,
        JSON.stringify(state.collapsedDeviceCards || {})
      );
    }

    function loadCollapsedDeviceCards() {
      const stored = safeParseJson(localStorage.getItem(DEVICE_COLLAPSE_STORAGE_KEY), {});
      if (stored && typeof stored === "object" && !Array.isArray(stored)) {
        state.collapsedDeviceCards = stored;
      }
    }

    function makeRealtimeRequestId(prefix = "cmd") {
      const rand = Math.random().toString(36).slice(2, 8);
      return `${prefix}-${Date.now()}-${rand}`;
    }

    function currentUserOwnsDevice(device) {
      if (!device || typeof device !== "object") {
        return false;
      }
      if (isAdminUser()) {
        return true;
      }
      if (device.is_owner === true) {
        return true;
      }
      const userId = state.user && typeof state.user.id === "string" ? state.user.id : "";
      return Boolean(userId && typeof device.owner_user_id === "string" && device.owner_user_id === userId);
    }

    function rejectPendingRealtimeCommands(reason) {
      const suffix = reason ? ` (${reason})` : "";
      for (const [requestId, pending] of state.pendingRealtimeCommands.entries()) {
        clearTimeout(pending.timer);
        pending.reject(new Error(`realtime_disconnected${suffix}`));
        state.pendingRealtimeCommands.delete(requestId);
      }
    }

    function resolveRealtimeCommandAck(message) {
      const requestId = typeof message.request_id === "string" ? message.request_id : "";
      if (!requestId) {
        return false;
      }
      const pending = state.pendingRealtimeCommands.get(requestId);
      if (!pending) {
        return false;
      }
      clearTimeout(pending.timer);
      state.pendingRealtimeCommands.delete(requestId);

      if (message.ok === false) {
        const code = typeof message.code === "string" ? message.code : "command_failed";
        const detail = typeof message.message === "string" ? message.message : "";
        pending.reject(new Error(`${code}${detail ? `: ${detail}` : ""}`));
        return true;
      }

      pending.resolve(message.result ?? null);
      return true;
    }

    function sendRealtimeCommand(commandPayload, options = {}) {
      const socket = state.realtimeSocket;
      if (!socket || socket.readyState !== WebSocket.OPEN || !state.realtimeAuthed) {
        throw new Error("Realtime channel is not connected.");
      }

      const timeoutMs =
        typeof options.timeoutMs === "number" && Number.isFinite(options.timeoutMs)
          ? Math.max(2000, Math.floor(options.timeoutMs))
          : 32000;
      const requestId =
        typeof options.requestId === "string" && options.requestId.trim()
          ? options.requestId.trim()
          : makeRealtimeRequestId("ws");
      const frame = {
        ...commandPayload,
        type: "cmd",
        request_id: requestId
      };

      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          state.pendingRealtimeCommands.delete(requestId);
          reject(new Error("command_timeout"));
        }, timeoutMs);

        state.pendingRealtimeCommands.set(requestId, {
          resolve,
          reject,
          timer
        });

        try {
          socket.send(JSON.stringify(frame));
        } catch (error) {
          clearTimeout(timer);
          state.pendingRealtimeCommands.delete(requestId);
          reject(error instanceof Error ? error : new Error("realtime_send_failed"));
        }
      });
    }

    function persistSession() {
      const snapshot = {
        base_url: baseUrl(),
        email: $("email").value.trim(),
        user: state.user,
        access_token: state.accessToken,
        refresh_token: state.refreshToken
      };
      localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(snapshot));
    }

    function clearSession() {
      state.user = null;
      state.accessToken = "";
      state.refreshToken = "";
      state.preferences = null;
      state.automationTestingByDevice.clear();
      state.irCodesByDevice.clear();
      state.irLastResultByDevice.clear();
      state.irFormByDevice.clear();
      persistSession();
    }

    function restoreSession() {
      const snapshot = safeParseJson(localStorage.getItem(SESSION_STORAGE_KEY), null);
      if (!snapshot || typeof snapshot !== "object") {
        return;
      }
      if (typeof snapshot.base_url === "string" && snapshot.base_url.trim()) {
        $("baseUrl").value = snapshot.base_url.trim();
      }
      if (typeof snapshot.email === "string" && snapshot.email.trim()) {
        $("email").value = snapshot.email.trim();
      }
      if (typeof snapshot.access_token === "string") {
        state.accessToken = snapshot.access_token;
      }
      if (typeof snapshot.refresh_token === "string") {
        state.refreshToken = snapshot.refresh_token;
      }
      if (snapshot.user && typeof snapshot.user === "object") {
        state.user = snapshot.user;
      }
    }

    function findDirectHeader(card) {
      for (const child of card.children) {
        if (child.classList && child.classList.contains("section-title")) {
          return child;
        }
      }
      return null;
    }

    function ensureCardHeader(card) {
      let header = findDirectHeader(card);
      if (header) {
        return header;
      }

      let title = null;
      for (const child of card.children) {
        if (child.tagName === "H1" || child.tagName === "H2") {
          title = child;
          break;
        }
      }
      if (!title) {
        return null;
      }

      header = document.createElement("div");
      header.className = "section-title";
      card.insertBefore(header, title);
      header.appendChild(title);
      return header;
    }

    function applyCardCollapsed(card, collapsed) {
      const header = findDirectHeader(card);
      for (const child of card.children) {
        if (child === header) {
          continue;
        }
        child.hidden = collapsed;
      }
      card.classList.toggle("is-collapsed", collapsed);
      const toggle = header ? header.querySelector(".collapse-toggle") : null;
      if (toggle) {
        toggle.textContent = collapsed ? "Expand" : "Collapse";
      }
    }

    function applyCollapseToAllCards() {
      document.querySelectorAll(".card").forEach((card, idx) => {
        const cardId = card.getAttribute("data-card-id") || `card-${idx + 1}`;
        applyCardCollapsed(card, Boolean(state.collapsedCards[cardId]));
      });
    }

    function setupCollapsibleCards() {
      document.querySelectorAll(".card").forEach((card, idx) => {
        const cardId = card.getAttribute("data-card-id") || `card-${idx + 1}`;
        const header = ensureCardHeader(card);
        if (!header) {
          return;
        }

        let toggle = header.querySelector(".collapse-toggle");
        if (!toggle) {
          toggle = document.createElement("button");
          toggle.type = "button";
          toggle.className = "collapse-toggle";
          toggle.addEventListener("click", () => {
            const nextCollapsed = !Boolean(state.collapsedCards[cardId]);
            state.collapsedCards[cardId] = nextCollapsed;
            persistCollapsedCards();
            applyCardCollapsed(card, nextCollapsed);
          });
          header.appendChild(toggle);
        }

        applyCardCollapsed(card, Boolean(state.collapsedCards[cardId]));
      });
    }

    function setAuthState() {
      const badge = $("authState");
      if (!state.user) {
        badge.textContent = "Not Authenticated";
        badge.className = "pill";
        $("tokenView").textContent = "";
        applyRoleVisibility();
        return;
      }
      badge.textContent = `${state.user.email} (${state.user.role})`;
      badge.className = "pill ok";
      const tokenTail = state.accessToken ? state.accessToken.slice(-16) : "";
      $("tokenView").textContent = tokenTail
        ? `access_token: ...${tokenTail} | refresh loaded: ${state.refreshToken ? "yes" : "no"}`
        : "refresh loaded: yes";
      applyRoleVisibility();
    }

    function isAdminUser() {
      return Boolean(state.user && state.user.role === "admin");
    }

    function applyRoleVisibility() {
      const admin = isAdminUser();
      document.querySelectorAll(".card[data-card-id]").forEach((card) => {
        const cardId = card.getAttribute("data-card-id");
        if (!cardId || !ADMIN_ONLY_CARD_IDS.has(cardId)) {
          return;
        }
        card.style.display = admin ? "" : "none";
      });
    }

    function buildDeviceViewState() {
      const view = {};
      for (const device of state.devices) {
        if (!device || !device.device_uid) continue;
        view[device.device_uid] = {
          last_seen_at: device.last_seen_at || null,
          is_active: Boolean(device.is_active),
          relays: Array.isArray(device.relays)
            ? device.relays.map((relay) => ({
                relay_index: relay.relay_index,
                is_on: relay.is_on
              }))
            : []
        };
      }
      return view;
    }

    function applyPreferences(prefs) {
      if (!prefs || typeof prefs !== "object") return;
      const settings = prefs.dashboard_settings || {};
      if (typeof settings.base_url === "string" && settings.base_url.trim()) {
        $("baseUrl").value = settings.base_url.trim();
      }
      if (typeof settings.last_email === "string" && settings.last_email.trim()) {
        $("email").value = settings.last_email.trim();
      }
      if (typeof settings.audit_source_filter === "string") {
        $("auditFilterSource").value = settings.audit_source_filter;
      }
      if (typeof settings.audit_action_filter === "string") {
        $("auditFilterAction").value = settings.audit_action_filter;
      }
      if (typeof settings.restore_path === "string") {
        $("restorePath").value = settings.restore_path;
      }
      if (typeof settings.release_model === "string" && settings.release_model.trim()) {
        $("releaseModel").value = settings.release_model.trim();
      }
      if (typeof settings.release_channel === "string" && settings.release_channel.trim()) {
        $("releaseChannel").value = settings.release_channel.trim();
      }
      if (
        settings.collapsed_cards &&
        typeof settings.collapsed_cards === "object" &&
        !Array.isArray(settings.collapsed_cards)
      ) {
        state.collapsedCards = {
          ...state.collapsedCards,
          ...settings.collapsed_cards
        };
        persistCollapsedCards();
        applyCollapseToAllCards();
      }
    }

    async function loadPreferences() {
      const prefs = await api("/api/v1/preferences", { method: "GET" });
      state.preferences = prefs;
      applyPreferences(prefs);
      persistSession();
      log("Loaded user preferences.");
    }

    async function savePreferences() {
      const payload = {
        merge: true,
        dashboard_layout: {
          pinned_sections: ["overview", "users", "devices", "ota", "backup", "audit", "metrics"]
        },
        dashboard_settings: {
          base_url: baseUrl(),
          last_email: $("email").value.trim(),
          audit_source_filter: $("auditFilterSource").value || "",
          audit_action_filter: $("auditFilterAction").value || "",
          restore_path: $("restorePath").value || "",
          release_model: $("releaseModel").value || "",
          release_channel: $("releaseChannel").value || "stable",
          collapsed_cards: state.collapsedCards
        },
        device_view_state: buildDeviceViewState(),
        notification_settings: {
          alert_simulation_enabled: true
        }
      };
      const saved = await api("/api/v1/preferences", {
        method: "PATCH",
        body: JSON.stringify(payload)
      });
      state.preferences = saved;
      persistSession();
      log("Saved user preferences.");
    }

    function summarizeHtmlError(rawText) {
      const titleMatch = rawText.match(/<title>([^<]+)<\/title>/i);
      const rayMatch = rawText.match(/Cloudflare Ray ID:\s*<strong[^>]*>([^<]+)<\/strong>/i);
      const title = titleMatch ? titleMatch[1].replace(/\s+/g, " ").trim() : "";
      const rayId = rayMatch ? rayMatch[1].trim() : "";

      if (/cloudflare/i.test(rawText)) {
        const titlePart = title || "Cloudflare gateway timeout";
        const rayPart = rayId ? ` Ray ID: ${rayId}.` : "";
        return `${titlePart}.${rayPart} Origin timed out or returned 504 (often device ACK timeout).`;
      }

      const compact = rawText
        .replace(/<script[\s\S]*?<\/script>/gi, " ")
        .replace(/<style[\s\S]*?<\/style>/gi, " ")
        .replace(/<[^>]+>/g, " ")
        .replace(/\s+/g, " ")
        .trim();
      return compact.slice(0, 240);
    }

    function toApiError(response, body, rawText) {
      if (body && typeof body === "object" && body.message) {
        const detailsText =
          body.details && typeof body.details === "object"
            ? ` details=${JSON.stringify(body.details).slice(0, 180)}`
            : "";
        return new Error(`${body.code || "error"}: ${body.message}${detailsText}`);
      }
      if (rawText) {
        const htmlLike = /^\s*<!doctype html/i.test(rawText) || /^\s*<html/i.test(rawText);
        if (htmlLike) {
          return new Error(`HTTP ${response.status}: ${summarizeHtmlError(rawText)}`);
        }
        return new Error(`HTTP ${response.status}: ${rawText.slice(0, 240)}`);
      }
      return new Error(`HTTP ${response.status}`);
    }

    async function rawRequest(path, options = {}, authToken = "") {
      const headers = {
        ...(options.headers || {})
      };
      const hasBody = Object.prototype.hasOwnProperty.call(options, "body");
      const isFormDataBody = hasBody && typeof FormData !== "undefined" && options.body instanceof FormData;
      if (hasBody && !isFormDataBody && !headers["content-type"] && !headers["Content-Type"]) {
        headers["content-type"] = "application/json";
      }
      if (authToken) {
        headers.authorization = `Bearer ${authToken}`;
      }

      const response = await fetch(baseUrl() + path, {
        ...options,
        headers
      });
      const text = await response.text();
      const body = safeParseJson(text, {});
      return { response, body, text };
    }

    async function refreshAccessToken() {
      if (!state.refreshToken) {
        throw new Error("No refresh token loaded.");
      }
      if (state.refreshPromise) {
        return state.refreshPromise;
      }

      state.refreshPromise = (async () => {
        const { response, body, text } = await rawRequest("/api/v1/auth/refresh", {
          method: "POST",
          body: JSON.stringify({ refresh_token: state.refreshToken })
        });

        if (!response.ok) {
          throw toApiError(response, body, text);
        }

        if (!body.access_token) {
          throw new Error("refresh_failed: Missing access_token.");
        }

        state.accessToken = body.access_token;
        state.refreshToken = body.refresh_token || state.refreshToken;
        setAuthState();
        persistSession();
        log("Access token refreshed.");
        if (state.user) {
          if (state.realtimeSocket && state.realtimeSocket.readyState === WebSocket.OPEN) {
            sendRealtimeAuth();
          } else {
            connectRealtime();
          }
        }
      })();

      try {
        await state.refreshPromise;
      } finally {
        state.refreshPromise = null;
      }
    }

    async function api(path, options = {}, allowRetry = true) {
      const first = await rawRequest(path, options, state.accessToken);
      if (first.response.ok) {
        return first.body;
      }

      const isAuthPath = path.startsWith("/api/v1/auth/");
      const canRetry =
        allowRetry &&
        first.response.status === 401 &&
        Boolean(state.refreshToken) &&
        !isAuthPath;

      if (canRetry) {
        try {
          await refreshAccessToken();
          const retry = await rawRequest(path, options, state.accessToken);
          if (!retry.response.ok) {
            throw toApiError(retry.response, retry.body, retry.text);
          }
          return retry.body;
        } catch (error) {
          clearSession();
          setAuthState();
          throw error instanceof Error
            ? error
            : new Error("unauthorized: Authentication required.");
        }
      }

      throw toApiError(first.response, first.body, first.text);
    }

    function renderKpis(overview) {
      const devices = overview.fleet?.devices || {};
      const users = overview.fleet?.users || {};
      const schedules = overview.schedules || {};
      const ota = overview.ota || {};

      const cards = [
        ["Devices Total", devices.total || 0],
        ["Devices Online", devices.online_estimate || 0],
        ["Users Active", users.active || 0],
        ["Schedules Enabled", schedules.enabled || 0],
        ["OTA Active Releases", ota.active_releases || 0],
        ["OTA Failed Reports 24h", ota.failed_reports_24h || 0],
        ["Claimed Devices", devices.claimed || 0],
        ["Unclaimed Devices", devices.unclaimed || 0]
      ];

      $("kpis").innerHTML = cards.map(([label, value]) =>
        `<div class="kpi"><div class="label">${label}</div><div class="value">${value}</div></div>`
      ).join("");
      $("generatedAt").textContent = `Generated ${formatIsoWithOffset(overview.generated_at)} (${DASHBOARD_TZ_LABEL})`;
      $("overviewMeta").textContent = `REST ${overview.api_versions?.rest || "v1"} | WS ${overview.api_versions?.ws || "v1"} | Deprecation window ${overview.api_versions?.deprecation_window_days || 0} days`;
      const policy = overview.backup?.policy || {};
      $("backupPolicy").textContent = `Backup dir: ${policy.output_dir || ""} | retention: ${policy.retention_count || 0} | encrypted: ${policy.encryption_configured ? "yes" : "no"} | RPO: ${policy.rpo_minutes || 0} min | RTO: ${policy.rto_minutes || 0} min`;
    }

    function renderUsers(users) {
      const rows = users.map((u) => `
        <tr>
          <td>${escapeHtml(u.email)}</td>
          <td><input data-user-name="${escapeHtml(u.id)}" value="${escapeHtml(u.name || "")}" /></td>
          <td>
            <select data-user-role="${escapeHtml(u.id)}">
              <option value="user" ${u.role === "user" ? "selected" : ""}>user</option>
              <option value="admin" ${u.role === "admin" ? "selected" : ""}>admin</option>
            </select>
          </td>
          <td>${escapeHtml(u.device_count)}</td>
          <td><input type="checkbox" data-user-active="${escapeHtml(u.id)}" ${u.is_active ? "checked" : ""} /></td>
          <td>
            <div class="row">
              <button data-user-save="${escapeHtml(u.id)}">Save</button>
              <button data-user-delete="${escapeHtml(u.id)}" class="danger" ${state.user?.id === u.id ? "disabled" : ""}>Delete</button>
            </div>
          </td>
        </tr>
      `).join("");

      $("usersTable").innerHTML = `
        <table>
          <thead><tr><th>Email</th><th>Name</th><th>Role</th><th>Devices</th><th>Active</th><th>Action</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      `;
    }

    function describeInputConfigRow(row) {
      if (!row || typeof row !== "object") {
        return "-";
      }
      const inputIdx = Number.isInteger(row.input_index) ? `B${row.input_index}` : "B?";
      const inputType = typeof row.input_type === "string" ? row.input_type : "unknown";
      const invert = row.invert_input === true ? "invert" : "";
      const hold = Number.isInteger(row.hold_seconds) ? `hold:${row.hold_seconds}s` : "";
      const rocker = typeof row.rocker_mode === "string" ? row.rocker_mode : "";

      if (row.linked !== true) {
        const extras = [invert, hold, rocker].filter(Boolean).join(" ");
        return extras ? `${inputIdx} ${inputType} (unlinked, ${extras})` : `${inputIdx} ${inputType} (unlinked)`;
      }

      const targetRelay = Number.isInteger(row.target_relay_index) ? `R${row.target_relay_index}` : "R?";
      const linkedExtras = [invert, hold, rocker].filter(Boolean).join(" ");
      return linkedExtras
        ? `${inputIdx} ${inputType} -> ${targetRelay} (${linkedExtras})`
        : `${inputIdx} ${inputType} -> ${targetRelay}`;
    }

    function normalizedInputConfig(device) {
      const buttonCount = Number.isInteger(device?.button_count) ? device.button_count : 0;
      const relayCount = Number.isInteger(device?.relay_count) ? device.relay_count : 0;
      const existingRows = Array.isArray(device?.input_config) ? device.input_config : [];
      const byIndex = new Map();
      for (const row of existingRows) {
        if (!row || typeof row !== "object") {
          continue;
        }
        if (!Number.isInteger(row.input_index)) {
          continue;
        }
        byIndex.set(row.input_index, row);
      }

      const normalized = [];
      for (let i = 0; i < buttonCount; i += 1) {
        const row = byIndex.get(i) || {};
        const inputType = row.input_type === "rocker_switch" ? "rocker_switch" : "push_button";
        const linked = row.linked !== false;
        const relayFallback = relayCount > 0 ? Math.min(i, relayCount - 1) : null;
        const targetRelay = Number.isInteger(row.target_relay_index)
          ? Math.min(Math.max(Number(row.target_relay_index), 0), Math.max(relayCount - 1, 0))
          : relayFallback;

        normalized.push({
          input_index: i,
          input_type: inputType,
          linked: relayCount > 0 ? linked : false,
          target_relay_index: relayCount > 0 && linked ? targetRelay : null,
          rocker_mode: inputType === "rocker_switch"
            ? (row.rocker_mode === "follow_position" ? "follow_position" : "edge_toggle")
            : null,
          invert_input: row.invert_input === true,
          hold_seconds: inputType === "push_button" && Number.isInteger(row.hold_seconds)
            ? Number(row.hold_seconds)
            : null
        });
      }

      return normalized;
    }

    function asConfigObject(value) {
      if (!value || typeof value !== "object" || Array.isArray(value)) {
        return {};
      }
      return value;
    }

    function normalizeConnectivityMode(value) {
      if (typeof value !== "string") {
        return "cloud_ws";
      }
      const normalized = value.trim().toLowerCase();
      if (normalized === "local_mqtt" || normalized === "mqtt" || normalized === "ha") {
        return "local_mqtt";
      }
      return "cloud_ws";
    }

    function normalizedConnectivityConfig(device) {
      const configRoot = asConfigObject(device?.config);
      const connectivity = asConfigObject(configRoot.connectivity || configRoot.connection);
      const mqtt = asConfigObject(connectivity.mqtt || connectivity.local_mqtt || configRoot.local_mqtt);
      const rawPort = Number.parseInt(String(mqtt.port ?? ""), 10);
      const port = Number.isInteger(rawPort) && rawPort > 0 && rawPort <= 65535 ? rawPort : 1883;
      const mode = normalizeConnectivityMode(
        connectivity.mode || connectivity.connection_mode || connectivity.transport_mode
      );
      return {
        mode,
        mqtt: {
          enabled: typeof mqtt.enabled === "boolean" ? mqtt.enabled : true,
          host: typeof mqtt.host === "string" ? mqtt.host.trim() : "",
          port,
          username: typeof mqtt.username === "string"
            ? mqtt.username
            : (typeof mqtt.user === "string" ? mqtt.user : ""),
          password: typeof mqtt.password === "string"
            ? mqtt.password
            : (typeof mqtt.pass === "string" ? mqtt.pass : ""),
          discovery_prefix: typeof mqtt.discovery_prefix === "string" && mqtt.discovery_prefix.trim()
            ? mqtt.discovery_prefix.trim()
            : "homeassistant",
          base_topic: typeof mqtt.base_topic === "string" && mqtt.base_topic.trim()
            ? mqtt.base_topic.trim()
            : "d"
        }
      };
    }

    function buildConnectivityEditor(device) {
      const cfg = normalizedConnectivityConfig(device);
      const canEdit = isAdminUser() || Boolean(device?.is_owner);
      const mode = cfg.mode === "local_mqtt" ? "local_mqtt" : "cloud_ws";
      const mqttEditable = canEdit && mode === "local_mqtt";
      const lockAttr = canEdit ? "" : ' data-conn-lock="1"';
      const modeLockAttr = canEdit ? "" : ' data-conn-lock="1"';
      return `
        <div class="conn-editor">
          <div class="conn-grid">
            <div>
              <label>Control Mode</label>
              <select data-conn-mode="${escapeHtml(device.id)}"${modeLockAttr} ${canEdit ? "" : "disabled"}>
                <option value="cloud_ws" ${mode === "cloud_ws" ? "selected" : ""}>Cloud (Hexa Server)</option>
                <option value="local_mqtt" ${mode === "local_mqtt" ? "selected" : ""}>Local MQTT (Home Assistant)</option>
              </select>
            </div>
            <div>
              <label>MQTT Port</label>
              <input data-conn-port="${escapeHtml(device.id)}" type="number" min="1" max="65535" value="${escapeHtml(cfg.mqtt.port)}"${lockAttr} ${mqttEditable ? "" : "disabled"} />
            </div>
            <div>
              <label>MQTT Host</label>
              <input data-conn-host="${escapeHtml(device.id)}" value="${escapeHtml(cfg.mqtt.host)}"${lockAttr} ${mqttEditable ? "" : "disabled"} />
            </div>
            <div>
              <label>MQTT Username</label>
              <input data-conn-username="${escapeHtml(device.id)}" value="${escapeHtml(cfg.mqtt.username)}"${lockAttr} ${mqttEditable ? "" : "disabled"} />
            </div>
            <div>
              <label>MQTT Password</label>
              <input data-conn-password="${escapeHtml(device.id)}" type="password" value="${escapeHtml(cfg.mqtt.password)}"${lockAttr} ${mqttEditable ? "" : "disabled"} />
            </div>
            <div>
              <label>Discovery Prefix</label>
              <input data-conn-discovery="${escapeHtml(device.id)}" value="${escapeHtml(cfg.mqtt.discovery_prefix)}"${lockAttr} ${mqttEditable ? "" : "disabled"} />
            </div>
            <div>
              <label>Base Topic</label>
              <input data-conn-base-topic="${escapeHtml(device.id)}" value="${escapeHtml(cfg.mqtt.base_topic)}"${lockAttr} ${mqttEditable ? "" : "disabled"} />
            </div>
            <label class="io-inline-check">
              <input data-conn-enabled="${escapeHtml(device.id)}" type="checkbox" ${cfg.mqtt.enabled ? "checked" : ""}${lockAttr} ${mqttEditable ? "" : "disabled"} />
              enable local MQTT
            </label>
          </div>
          ${canEdit
            ? '<div class="meta">Changing mode may restart device networking briefly.</div>'
            : '<div class="meta">Read-only. Only owner/admin can change connectivity mode.</div>'}
        </div>
      `;
    }

    function syncConnectivityControls(deviceId) {
      const modeEl = document.querySelector(`[data-conn-mode="${deviceId}"]`);
      if (!(modeEl instanceof HTMLSelectElement)) {
        return;
      }
      if (modeEl.dataset.connLock === "1") {
        return;
      }

      const localMode = modeEl.value === "local_mqtt";
      const selectors = [
        `[data-conn-enabled="${deviceId}"]`,
        `[data-conn-host="${deviceId}"]`,
        `[data-conn-port="${deviceId}"]`,
        `[data-conn-username="${deviceId}"]`,
        `[data-conn-password="${deviceId}"]`,
        `[data-conn-discovery="${deviceId}"]`,
        `[data-conn-base-topic="${deviceId}"]`
      ];
      for (const selector of selectors) {
        const field = document.querySelector(selector);
        if (!(field instanceof HTMLInputElement || field instanceof HTMLSelectElement)) {
          continue;
        }
        if (field.dataset.connLock === "1") {
          field.disabled = true;
          continue;
        }
        field.disabled = !localMode;
      }
    }

    function collectConnectivityConfigFromEditor(device) {
      const deviceId = device?.id;
      if (!deviceId) {
        throw new Error("Device id is required.");
      }

      const modeEl = document.querySelector(`[data-conn-mode="${deviceId}"]`);
      const enabledEl = document.querySelector(`[data-conn-enabled="${deviceId}"]`);
      const hostEl = document.querySelector(`[data-conn-host="${deviceId}"]`);
      const portEl = document.querySelector(`[data-conn-port="${deviceId}"]`);
      const usernameEl = document.querySelector(`[data-conn-username="${deviceId}"]`);
      const passwordEl = document.querySelector(`[data-conn-password="${deviceId}"]`);
      const discoveryEl = document.querySelector(`[data-conn-discovery="${deviceId}"]`);
      const baseTopicEl = document.querySelector(`[data-conn-base-topic="${deviceId}"]`);

      if (!(modeEl instanceof HTMLSelectElement)) throw new Error("Missing connectivity mode control.");
      if (!(enabledEl instanceof HTMLInputElement)) throw new Error("Missing MQTT enabled control.");
      if (!(hostEl instanceof HTMLInputElement)) throw new Error("Missing MQTT host control.");
      if (!(portEl instanceof HTMLInputElement)) throw new Error("Missing MQTT port control.");
      if (!(usernameEl instanceof HTMLInputElement)) throw new Error("Missing MQTT username control.");
      if (!(passwordEl instanceof HTMLInputElement)) throw new Error("Missing MQTT password control.");
      if (!(discoveryEl instanceof HTMLInputElement)) throw new Error("Missing discovery prefix control.");
      if (!(baseTopicEl instanceof HTMLInputElement)) throw new Error("Missing base topic control.");

      const mode = modeEl.value === "local_mqtt" ? "local_mqtt" : "cloud_ws";
      const host = hostEl.value.trim();
      const portParsed = Number.parseInt(portEl.value, 10);
      if (!Number.isInteger(portParsed) || portParsed < 1 || portParsed > 65535) {
        throw new Error("MQTT port must be between 1 and 65535.");
      }

      if (mode === "local_mqtt" && !host) {
        throw new Error("MQTT host is required in local MQTT mode.");
      }

      const mqtt = {
        enabled: enabledEl.checked,
        host,
        port: portParsed,
        username: usernameEl.value,
        discovery_prefix: discoveryEl.value.trim() || "homeassistant",
        base_topic: baseTopicEl.value.trim() || "d"
      };
      if (passwordEl.value.length > 0) {
        mqtt.password = passwordEl.value;
      }

      return {
        mode,
        mqtt
      };
    }

    function mergeConnectivityConfig(device, connectivityPayload) {
      const root = asConfigObject(safeParseJson(JSON.stringify(asConfigObject(device?.config)), {}));
      const existingConnectivity = asConfigObject(root.connectivity);
      const existingMqtt = asConfigObject(existingConnectivity.mqtt);
      root.connectivity = {
        ...existingConnectivity,
        mode: connectivityPayload.mode,
        mqtt: {
          ...existingMqtt,
          ...connectivityPayload.mqtt
        }
      };
      return root;
    }

    function buildInputConfigEditor(device) {
      const relayNames = Array.isArray(device?.relay_names) ? device.relay_names : [];
      const relayCount = Number.isInteger(device?.relay_count) ? device.relay_count : 0;
      const normalized = normalizedInputConfig(device);

      if (!normalized.length) {
        return '<div class="muted">no buttons on this device</div>';
      }

      const rows = normalized.map((row) => {
        const key = `${device.id}:${row.input_index}`;
        const inputType = row.input_type === "rocker_switch" ? "rocker_switch" : "push_button";
        const linked = row.linked === true && relayCount > 0;
        const follow = inputType === "rocker_switch" && row.rocker_mode === "follow_position";
        const targetRelay = Number.isInteger(row.target_relay_index) ? row.target_relay_index : 0;

        const relayOptions = relayCount > 0
          ? Array.from({ length: relayCount }, (_, relayIndex) => {
              const relayName = typeof relayNames[relayIndex] === "string"
                ? relayNames[relayIndex]
                : `Relay ${relayIndex + 1}`;
              const selected = targetRelay === relayIndex ? "selected" : "";
              return `<option value="${relayIndex}" ${selected}>R${relayIndex} - ${escapeHtml(relayName)}</option>`;
            }).join("")
          : '<option value="">No relays</option>';

        const targetDisabled = relayCount === 0 || !linked;
        const followDisabled = inputType !== "rocker_switch";

        return `
          <div class="io-row">
            <div class="io-row-head">
              <strong>B${escapeHtml(row.input_index)}</strong>
              <span class="meta">${escapeHtml(describeInputConfigRow(row))}</span>
            </div>
            <div class="io-row-grid">
              <div>
                <label>Input Type</label>
                <select data-io-type="${escapeHtml(key)}">
                  <option value="push_button" ${inputType === "push_button" ? "selected" : ""}>push_button</option>
                  <option value="rocker_switch" ${inputType === "rocker_switch" ? "selected" : ""}>rocker_switch</option>
                </select>
              </div>
              <div>
                <label>Target Relay</label>
                <select data-io-target="${escapeHtml(key)}" ${targetDisabled ? "disabled" : ""}>
                  ${relayOptions}
                </select>
              </div>
            </div>
            <div class="row">
              <label class="io-inline-check">
                <input type="checkbox" data-io-linked="${escapeHtml(key)}" ${linked ? "checked" : ""} ${relayCount === 0 ? "disabled" : ""} />
                linked to relay
              </label>
              <label class="io-inline-check">
                <input type="checkbox" data-io-follow="${escapeHtml(key)}" ${follow ? "checked" : ""} ${followDisabled ? "disabled" : ""} />
                follow mode (rocker)
              </label>
            </div>
          </div>
        `;
      }).join("");

      return `<div class="io-editor">${rows}</div>`;
    }

    function syncInputRowControls(key) {
      const typeEl = document.querySelector(`[data-io-type="${key}"]`);
      const linkedEl = document.querySelector(`[data-io-linked="${key}"]`);
      const targetEl = document.querySelector(`[data-io-target="${key}"]`);
      const followEl = document.querySelector(`[data-io-follow="${key}"]`);

      if (!(typeEl instanceof HTMLSelectElement)) return;
      if (!(linkedEl instanceof HTMLInputElement)) return;
      if (!(targetEl instanceof HTMLSelectElement)) return;
      if (!(followEl instanceof HTMLInputElement)) return;

      const isRocker = typeEl.value === "rocker_switch";
      if (!isRocker) {
        followEl.checked = false;
      }
      followEl.disabled = !isRocker;

      const hasRelay = targetEl.options.length > 0 && targetEl.value !== "";
      targetEl.disabled = !linkedEl.checked || !hasRelay;
    }

    function collectInputConfigFromEditor(device) {
      const relayCount = Number.isInteger(device?.relay_count) ? device.relay_count : 0;
      const normalized = normalizedInputConfig(device);
      const byIndex = new Map(normalized.map((row) => [row.input_index, row]));

      const payload = [];
      for (let i = 0; i < normalized.length; i += 1) {
        const key = `${device.id}:${i}`;
        const typeEl = document.querySelector(`[data-io-type="${key}"]`);
        const linkedEl = document.querySelector(`[data-io-linked="${key}"]`);
        const targetEl = document.querySelector(`[data-io-target="${key}"]`);
        const followEl = document.querySelector(`[data-io-follow="${key}"]`);

        if (!(typeEl instanceof HTMLSelectElement)) {
          throw new Error(`Missing type control for button ${i}`);
        }
        if (!(linkedEl instanceof HTMLInputElement)) {
          throw new Error(`Missing linked control for button ${i}`);
        }
        if (!(targetEl instanceof HTMLSelectElement)) {
          throw new Error(`Missing relay control for button ${i}`);
        }
        if (!(followEl instanceof HTMLInputElement)) {
          throw new Error(`Missing follow control for button ${i}`);
        }

        const current = byIndex.get(i) || {};
        const inputType = typeEl.value === "rocker_switch" ? "rocker_switch" : "push_button";
        const linked = relayCount > 0 ? linkedEl.checked : false;
        const targetRelayParsed = Number.parseInt(targetEl.value, 10);
        const targetRelay =
          linked && relayCount > 0 && Number.isInteger(targetRelayParsed)
            ? Math.min(Math.max(targetRelayParsed, 0), relayCount - 1)
            : null;

        payload.push({
          input_index: i,
          input_type: inputType,
          linked,
          target_relay_index: targetRelay,
          rocker_mode: inputType === "rocker_switch"
            ? (followEl.checked ? "follow_position" : "edge_toggle")
            : null,
          invert_input: current.invert_input === true,
          hold_seconds: inputType === "push_button" && Number.isInteger(current.hold_seconds)
            ? Number(current.hold_seconds)
            : null
        });
      }

      return payload;
    }

    function summarizeLastAction(device) {
      const action = device && device.last_action && typeof device.last_action === "object" ? device.last_action : null;
      if (!action) {
        return "-";
      }
      const actionName = typeof action.action === "string" ? action.action : "unknown";
      const source = typeof action.source === "string" ? action.source : "n/a";
      const at = typeof action.created_at === "string" ? formatIsoWithOffset(action.created_at) : "-";
      const details = action.details && typeof action.details === "object" ? action.details : {};
      const relayPart = Number.isInteger(details.relay_index) ? ` r${details.relay_index}` : "";
      const commandPart = typeof details.action === "string" ? ` ${details.action}` : "";
      const suffix = [relayPart, commandPart].join("").trim();
      return `${actionName}${suffix ? ` (${suffix})` : ""} [${source}] ${at !== "-" ? at : ""}`.trim();
    }

    function summarizeLastInputEvent(device) {
      const inputEvent = device && device.last_input_event && typeof device.last_input_event === "object"
        ? device.last_input_event
        : null;
      if (!inputEvent) {
        return "-";
      }
      const details = inputEvent.details && typeof inputEvent.details === "object" ? inputEvent.details : {};
      const idx = Number.isInteger(details.input_index) ? `B${details.input_index}` : "B?";
      const eventName = typeof details.event === "string" ? details.event : "event";
      const duration = Number.isFinite(Number(details.duration_ms)) ? `${Number(details.duration_ms)}ms` : "";
      const at = typeof inputEvent.created_at === "string" ? formatIsoWithOffset(inputEvent.created_at) : "-";
      return `${idx} ${eventName}${duration ? ` (${duration})` : ""} ${at !== "-" ? at : ""}`.trim();
    }

    function renderButtonStates(device) {
      const buttonCount = Number.isInteger(device?.button_count) ? device.button_count : 0;
      if (buttonCount <= 0) {
        return '<div class="muted">No buttons configured.</div>';
      }

      const states = ensureDeviceButtonStates(device);
      const chips = [];
      for (let i = 0; i < buttonCount; i += 1) {
        const row = states[String(i)] && typeof states[String(i)] === "object" ? states[String(i)] : {};
        const rawState = row.state === "pressed" || row.state === "released" ? row.state : "unknown";
        const stateLabel = rawState === "pressed" ? "Pressed" : rawState === "released" ? "Released" : "Unknown";
        const updatedAt = typeof row.ts === "string" ? formatIsoWithOffset(row.ts) : "-";
        chips.push(`
          <div class="button-state-chip ${rawState}">
            <strong>B${escapeHtml(i)}</strong>
            <span>${escapeHtml(stateLabel)}</span>
            <div class="meta">${updatedAt !== "-" ? escapeHtml(updatedAt) : "No signal yet"}</div>
          </div>
        `);
      }

      return `<div class="button-state-grid">${chips.join("")}</div>`;
    }

    function asCapabilitySummaryList(value) {
      if (!Array.isArray(value)) {
        return [];
      }
      const out = [];
      for (const item of value) {
        if (!item || typeof item !== "object" || Array.isArray(item)) {
          continue;
        }
        const key = typeof item.key === "string" ? item.key : "";
        const kind = typeof item.kind === "string" ? item.kind : "";
        if (!key || !kind) {
          continue;
        }
        out.push({
          key,
          kind,
          enabled: item.enabled !== false
        });
      }
      return out;
    }

    function isIrCapableDevice(device) {
      if (!device || typeof device !== "object") {
        return false;
      }
      if (device.device_class === "ir_hub" || device.device_class === "hybrid") {
        return true;
      }
      const capabilities = asCapabilitySummaryList(device.capabilities);
      return capabilities.some(
        (item) =>
          item.enabled &&
          (item.key === "ir_tx" || item.key === "ir_rx" || item.kind === "infrared")
      );
    }

    function getIrFormState(deviceId) {
      const key = String(deviceId || "").trim();
      const cached = key ? state.irFormByDevice.get(key) : null;
      return {
        learnType: cached && cached.learnType === "ac" ? "ac" : "button",
        codeName: cached && typeof cached.codeName === "string" ? cached.codeName : "",
        topN: cached && typeof cached.topN === "string" && cached.topN.trim().length > 0
          ? cached.topN
          : "5",
        candidateProtocol:
          cached && typeof cached.candidateProtocol === "string" ? cached.candidateProtocol : "",
        candidateFrequency:
          cached && typeof cached.candidateFrequency === "string" ? cached.candidateFrequency : "",
        candidateFormat:
          cached && typeof cached.candidateFormat === "string" ? cached.candidateFormat : "",
        candidateBrand:
          cached && typeof cached.candidateBrand === "string" ? cached.candidateBrand : "",
        candidateModel:
          cached && typeof cached.candidateModel === "string" ? cached.candidateModel : "",
        candidatePayload:
          cached && typeof cached.candidatePayload === "string" ? cached.candidatePayload : ""
      };
    }

    function setIrFormState(deviceId, patch) {
      const key = String(deviceId || "").trim();
      if (!key || !patch || typeof patch !== "object") {
        return;
      }
      const next = {
        ...getIrFormState(key),
        ...patch
      };
      next.learnType = next.learnType === "ac" ? "ac" : "button";
      if (typeof next.topN !== "string" || next.topN.trim().length === 0) {
        next.topN = "5";
      }
      state.irFormByDevice.set(key, next);
    }

    function getDeviceIdFromCardElement(element) {
      if (!(element instanceof Element)) {
        return "";
      }
      const card = element.closest(".device-card");
      if (!(card instanceof HTMLElement)) {
        return "";
      }
      return String(card.getAttribute("data-device-id") || "").trim();
    }

    function captureIrFormFromCard(deviceId, card) {
      const key = String(deviceId || "").trim();
      if (!key || !(card instanceof HTMLElement)) {
        return;
      }
      const learnTypeEl = card.querySelector("[data-ir-learn-type]");
      const codeNameEl = card.querySelector("[data-ir-code-name]");
      const topNEl = card.querySelector("[data-ir-top-n]");
      const protocolEl = card.querySelector("[data-ir-candidate-protocol]");
      const frequencyEl = card.querySelector("[data-ir-candidate-frequency]");
      const formatEl = card.querySelector("[data-ir-candidate-format]");
      const brandEl = card.querySelector("[data-ir-candidate-brand]");
      const modelEl = card.querySelector("[data-ir-candidate-model]");
      const payloadEl = card.querySelector("[data-ir-candidate-payload]");
      setIrFormState(key, {
        learnType:
          learnTypeEl instanceof HTMLSelectElement && learnTypeEl.value.trim().toLowerCase() === "ac"
            ? "ac"
            : "button",
        codeName: codeNameEl instanceof HTMLInputElement ? codeNameEl.value : "",
        topN: topNEl instanceof HTMLInputElement ? topNEl.value : "5",
        candidateProtocol: protocolEl instanceof HTMLInputElement ? protocolEl.value : "",
        candidateFrequency: frequencyEl instanceof HTMLInputElement ? frequencyEl.value : "",
        candidateFormat: formatEl instanceof HTMLInputElement ? formatEl.value : "",
        candidateBrand: brandEl instanceof HTMLInputElement ? brandEl.value : "",
        candidateModel: modelEl instanceof HTMLInputElement ? modelEl.value : "",
        candidatePayload: payloadEl instanceof HTMLTextAreaElement ? payloadEl.value : ""
      });
    }

    function syncIrFormStateFromElement(element) {
      if (!(element instanceof HTMLElement)) {
        return;
      }
      const deviceId = getDeviceIdFromCardElement(element);
      if (!deviceId) {
        return;
      }

      if (element.hasAttribute("data-ir-learn-type") ||
          element.hasAttribute("data-ir-code-name") ||
          element.hasAttribute("data-ir-top-n") ||
          element.hasAttribute("data-ir-candidate-protocol") ||
          element.hasAttribute("data-ir-candidate-frequency") ||
          element.hasAttribute("data-ir-candidate-format") ||
          element.hasAttribute("data-ir-candidate-brand") ||
          element.hasAttribute("data-ir-candidate-model") ||
          element.hasAttribute("data-ir-candidate-payload")) {
        const card = element.closest(".device-card");
        if (card instanceof HTMLElement) {
          captureIrFormFromCard(deviceId, card);
        }
      }
    }

    function getIrCodeCache(deviceId) {
      const key = String(deviceId || "").trim();
      const cached = state.irCodesByDevice.get(key);
      if (!cached || typeof cached !== "object") {
        return {
          loading: false,
          loadedAt: "",
          error: "",
          codes: [],
          selectedCodeId: ""
        };
      }
      return {
        loading: cached.loading === true,
        loadedAt: typeof cached.loadedAt === "string" ? cached.loadedAt : "",
        error: typeof cached.error === "string" ? cached.error : "",
        codes: Array.isArray(cached.codes) ? cached.codes : [],
        selectedCodeId: typeof cached.selectedCodeId === "string" ? cached.selectedCodeId : ""
      };
    }

    function setIrCodeSelection(deviceId, codeId) {
      const key = String(deviceId || "").trim();
      if (!key) {
        return;
      }
      const cached = getIrCodeCache(key);
      state.irCodesByDevice.set(key, {
        ...cached,
        selectedCodeId: String(codeId || "").trim()
      });
    }

    function getIrLastResult(deviceId) {
      const key = String(deviceId || "").trim();
      const cached = state.irLastResultByDevice.get(key);
      if (!cached || typeof cached !== "object") {
        return {
          message: "",
          level: "info",
          at: ""
        };
      }
      return {
        message: typeof cached.message === "string" ? cached.message : "",
        level: cached.level === "error" ? "error" : "info",
        at: typeof cached.at === "string" ? cached.at : ""
      };
    }

    function setIrLastResult(deviceId, message, level = "info") {
      const key = String(deviceId || "").trim();
      if (!key) {
        return;
      }
      state.irLastResultByDevice.set(key, {
        message: String(message || "").trim(),
        level: level === "error" ? "error" : "info",
        at: nowIso()
      });
      renderDevices(state.devices);
    }

    async function loadIrCodesForDevice(deviceId, options = {}) {
      const key = String(deviceId || "").trim();
      if (!key) {
        return [];
      }

      const force = options.force === true;
      const shouldRender = options.render !== false;
      const cached = getIrCodeCache(key);
      if (!force && cached.loading) {
        return cached.codes;
      }

      state.irCodesByDevice.set(key, {
        ...cached,
        loading: true,
        error: ""
      });
      if (shouldRender) {
        renderDevices(state.devices);
      }

      try {
        const rows = await api(`/api/v1/devices/${encodeURIComponent(key)}/ir-codes`, { method: "GET" });
        const codes = Array.isArray(rows) ? rows : [];
        const selectedCodeId = codes.some((entry) => entry && entry.id === cached.selectedCodeId)
          ? cached.selectedCodeId
          : typeof codes[0]?.id === "string"
            ? codes[0].id
            : "";

        state.irCodesByDevice.set(key, {
          loading: false,
          loadedAt: nowIso(),
          error: "",
          codes,
          selectedCodeId
        });
        return codes;
      } catch (error) {
        state.irCodesByDevice.set(key, {
          ...cached,
          loading: false,
          error: error instanceof Error ? error.message : "ir_code_load_failed"
        });
        throw error;
      } finally {
        if (shouldRender) {
          renderDevices(state.devices);
        }
      }
    }

    function findDeviceCardElement(deviceId) {
      const key = String(deviceId || "").trim();
      if (!key) {
        return null;
      }
      const cards = document.querySelectorAll(".device-card");
      for (const card of cards) {
        if (card instanceof HTMLElement && card.getAttribute("data-device-id") === key) {
          return card;
        }
      }
      return null;
    }

    function collectIrCandidateFromCard(card) {
      const protocolEl = card.querySelector("[data-ir-candidate-protocol]");
      const payloadEl = card.querySelector("[data-ir-candidate-payload]");
      const frequencyEl = card.querySelector("[data-ir-candidate-frequency]");
      const formatEl = card.querySelector("[data-ir-candidate-format]");
      const brandHintEl = card.querySelector("[data-ir-candidate-brand]");
      const modelHintEl = card.querySelector("[data-ir-candidate-model]");

      const protocol = protocolEl instanceof HTMLInputElement ? protocolEl.value.trim() : "";
      const payload = payloadEl instanceof HTMLTextAreaElement ? payloadEl.value.trim() : "";
      const frequencyRaw = frequencyEl instanceof HTMLInputElement ? frequencyEl.value.trim() : "";
      const payloadFormat = formatEl instanceof HTMLInputElement ? formatEl.value.trim() : "";
      const brandHint = brandHintEl instanceof HTMLInputElement ? brandHintEl.value.trim() : "";
      const modelHint = modelHintEl instanceof HTMLInputElement ? modelHintEl.value.trim() : "";

      const hasAny =
        Boolean(protocol) ||
        Boolean(payload) ||
        Boolean(frequencyRaw) ||
        Boolean(payloadFormat) ||
        Boolean(brandHint) ||
        Boolean(modelHint);
      if (!hasAny) {
        return null;
      }
      if (!protocol || !payload) {
        throw new Error("Candidate protocol and payload are required.");
      }

      const candidate = {
        protocol,
        payload
      };

      if (frequencyRaw) {
        const frequency = Number(frequencyRaw);
        if (!Number.isInteger(frequency) || frequency <= 0) {
          throw new Error("Candidate frequency must be a positive integer.");
        }
        candidate.frequency_hz = frequency;
      }
      if (payloadFormat) {
        candidate.payload_format = payloadFormat;
      }
      if (brandHint) {
        candidate.brand_hint = brandHint;
      }
      if (modelHint) {
        candidate.model_hint = modelHint;
      }

      return candidate;
    }

    function collectIrTopNFromCard(card) {
      const topNEl = card.querySelector("[data-ir-top-n]");
      const topNRaw = topNEl instanceof HTMLInputElement ? topNEl.value.trim() : "";
      if (!topNRaw) {
        return undefined;
      }
      const topN = Number(topNRaw);
      if (!Number.isInteger(topN) || topN < 1 || topN > 20) {
        throw new Error("Top N must be an integer between 1 and 20.");
      }
      return topN;
    }

    function renderIrControls(device, isOwner) {
      const deviceId = String(device?.id || "").trim();
      const cache = getIrCodeCache(deviceId);
      const irForm = getIrFormState(deviceId);
      const canControl = Boolean(isOwner);
      const disabled = canControl ? "" : "disabled";
      const hasCodes = cache.codes.length > 0;
      const selectedCodeId = cache.codes.some((entry) => entry && entry.id === cache.selectedCodeId)
        ? cache.selectedCodeId
        : typeof cache.codes[0]?.id === "string"
          ? cache.codes[0].id
          : "";
      const loadedAt = cache.loadedAt ? formatIsoWithOffset(cache.loadedAt) : "-";
      const lastResult = getIrLastResult(deviceId);
      const lastResultAt = lastResult.at ? formatIsoWithOffset(lastResult.at) : "-";

      const options = hasCodes
        ? cache.codes.map((code) => {
            const codeId = typeof code?.id === "string" ? code.id : "";
            const codeName = typeof code?.code_name === "string" ? code.code_name : codeId;
            const protocol = typeof code?.protocol === "string" ? code.protocol : "-";
            const selected = codeId && codeId === selectedCodeId ? "selected" : "";
            return `<option value="${escapeHtml(codeId)}" ${selected}>${escapeHtml(codeName)} (${escapeHtml(protocol)})</option>`;
          }).join("")
        : `<option value="">No IR codes loaded</option>`;

      return `
        <section class="device-block">
          <div class="device-block-title">IR Controls</div>

          <div class="grid cols-2">
            <div>
              <label>Learn Type</label>
              <select data-ir-learn-type ${disabled}>
                <option value="button" ${irForm.learnType === "button" ? "selected" : ""}>button</option>
                <option value="ac" ${irForm.learnType === "ac" ? "selected" : ""}>ac</option>
              </select>
            </div>
            <div>
              <label>Code Name</label>
              <input data-ir-code-name placeholder="power_toggle" value="${escapeHtml(irForm.codeName)}" ${disabled} />
            </div>
          </div>

          <div class="device-action-row">
            <button data-ir-action="learn_start" data-ir-device="${escapeHtml(deviceId)}" ${disabled}>Learn Start</button>
            <button data-ir-action="learn_test" data-ir-device="${escapeHtml(deviceId)}" class="secondary" ${disabled}>Learn Test</button>
            <button data-ir-action="learn_save" data-ir-device="${escapeHtml(deviceId)}" class="secondary" ${disabled}>Learn Save</button>
            <button data-ir-action="learn_discard" data-ir-device="${escapeHtml(deviceId)}" class="warn" ${disabled}>Learn Discard</button>
          </div>

          <div class="grid cols-2">
            <div>
              <label>Stored IR Codes</label>
              <select data-ir-code-select="${escapeHtml(deviceId)}" ${disabled}>
                ${options}
              </select>
            </div>
            <div>
              <label>Candidate Top N</label>
              <input data-ir-top-n type="number" min="1" max="20" value="${escapeHtml(irForm.topN)}" ${disabled} />
            </div>
          </div>

          <div class="device-action-row">
            <button data-ir-action="send_code" data-ir-device="${escapeHtml(deviceId)}" ${!canControl || !hasCodes ? "disabled" : ""}>Send Code</button>
            <button data-ir-action="delete_code" data-ir-device="${escapeHtml(deviceId)}" class="danger" ${!canControl || !hasCodes ? "disabled" : ""}>Delete Code</button>
            <button data-ir-action="load_codes" data-ir-device="${escapeHtml(deviceId)}" class="ghost" ${disabled}>${cache.loading ? "Loading..." : "Refresh Codes"}</button>
          </div>

          <div class="grid cols-3">
            <div>
              <label>Candidate Protocol</label>
              <input data-ir-candidate-protocol placeholder="NEC" value="${escapeHtml(irForm.candidateProtocol)}" ${disabled} />
            </div>
            <div>
              <label>Candidate Frequency (Hz)</label>
              <input data-ir-candidate-frequency type="number" min="1" step="1" placeholder="38000" value="${escapeHtml(irForm.candidateFrequency)}" ${disabled} />
            </div>
            <div>
              <label>Candidate Format</label>
              <input data-ir-candidate-format placeholder="raw" value="${escapeHtml(irForm.candidateFormat)}" ${disabled} />
            </div>
            <div>
              <label>Brand Hint</label>
              <input data-ir-candidate-brand placeholder="Samsung" value="${escapeHtml(irForm.candidateBrand)}" ${disabled} />
            </div>
            <div>
              <label>Model Hint</label>
              <input data-ir-candidate-model placeholder="TV-1234" value="${escapeHtml(irForm.candidateModel)}" ${disabled} />
            </div>
          </div>

          <div>
            <label>Candidate Payload</label>
            <textarea data-ir-candidate-payload placeholder="raw timings or encoded IR payload" ${disabled}>${escapeHtml(irForm.candidatePayload)}</textarea>
          </div>

          <div class="device-action-row">
            <button data-ir-action="recognize_candidate" data-ir-device="${escapeHtml(deviceId)}" class="secondary" ${disabled}>Recognize Candidate</button>
          </div>

          <div class="meta">codes: ${escapeHtml(cache.codes.length)} | loaded: ${escapeHtml(loadedAt)}</div>
          <div class="meta" ${lastResult.level === "error" ? 'style="color: var(--danger);"' : ""}>
            last_ir: ${escapeHtml(lastResult.message || "No IR action yet")} | at: ${escapeHtml(lastResultAt)}
          </div>
          ${cache.error ? `<div class="meta" style="color: var(--danger);">${escapeHtml(cache.error)}</div>` : ""}
          ${canControl ? "" : `<div class="meta">Only owner/admin can run IR commands.</div>`}
        </section>
      `;
    }

    function deviceMatchesSearch(device, query) {
      if (!query) {
        return true;
      }

      const relayNames = Array.isArray(device?.relay_names) ? device.relay_names.join(" ") : "";
      const fields = [
        device?.id,
        device?.name,
        device?.device_uid,
        device?.hardware_uid,
        device?.model,
        device?.device_class,
        device?.owner_email,
        device?.owner_user_id,
        device?.claim_code,
        device?.firmware_version,
        device?.ota_channel,
        device?.last_ip,
        relayNames
      ];
      return fields.some((field) => String(field ?? "").toLowerCase().includes(query));
    }

    function renderDevices(devices) {
      const allDevices = Array.isArray(devices) ? devices : [];
      if (allDevices.length === 0) {
        $("devicesTable").innerHTML = `
          <div class="devices-list">
            <div class="device-card">
              <div class="muted">No devices found.</div>
            </div>
          </div>
        `;
        return;
      }

      const searchQuery = String(state.deviceSearchQuery || "").trim().toLowerCase();
      const visibleDevices = searchQuery
        ? allDevices.filter((device) => deviceMatchesSearch(device, searchQuery))
        : allDevices;

      if (visibleDevices.length === 0) {
        $("devicesTable").innerHTML = `
          <div class="devices-list">
            <div class="device-card">
              <div class="muted">No devices match "${escapeHtml(String(state.deviceSearchQuery || "").trim())}".</div>
            </div>
          </div>
        `;
        return;
      }

      const cards = visibleDevices.map((d) => {
        const admin = isAdminUser();
        initializeDeviceButtonStates(d);
        const cardKey = String(d.id || d.device_uid || "");
        const isCollapsed = Boolean(state.collapsedDeviceCards[cardKey]);
        const isOwner = currentUserOwnsDevice(d);
        const irCapable = isIrCapableDevice(d);
        const showRelayControls = d.device_class !== "ir_hub";
        const relayNames = Array.isArray(d.relay_names) ? d.relay_names : [];
        const relayRows = Array.isArray(d.relays) && d.relays.length > 0
          ? d.relays
          : Array.from({ length: Math.max(Number(d.relay_count) || 0, 0) }, (_, relayIndex) => ({
              relay_index: relayIndex,
              relay_name: typeof relayNames[relayIndex] === "string" ? relayNames[relayIndex] : null,
              is_on: null
            }));

        const relayControls = relayRows.map((r) => {
          const relayIndex = Number(r.relay_index);
          const relayName = typeof relayNames[relayIndex] === "string"
            ? relayNames[relayIndex]
            : `Relay ${relayIndex + 1}`;
          const relayState = r.is_on === true ? "ON" : r.is_on === false ? "OFF" : "UNKNOWN";
          const relayBadgeClass = r.is_on === true ? "ok" : r.is_on === false ? "warn" : "";
          return `
            <div class="relay-card">
              <div class="relay-head">
                <strong>${escapeHtml(relayName)}</strong>
                <span class="pill ${relayBadgeClass}">R${escapeHtml(relayIndex)}:${escapeHtml(relayState)}</span>
              </div>
              <div class="device-action-row">
                <button data-relay="${escapeHtml(d.id)}:${escapeHtml(relayIndex)}:on">On</button>
                <button data-relay="${escapeHtml(d.id)}:${escapeHtml(relayIndex)}:off" class="secondary">Off</button>
                <button data-relay="${escapeHtml(d.id)}:${escapeHtml(relayIndex)}:toggle" class="warn">Toggle</button>
              </div>
            </div>
          `;
        }).join("");

        const inputEditor = buildInputConfigEditor(d);
        const connectivityEditor = buildConnectivityEditor(d);
        const connectivityConfig = normalizedConnectivityConfig(d);
        const buttonStates = renderButtonStates(d);
        const relayCount = Number.isInteger(d.relay_count) ? Math.max(Number(d.relay_count), 0) : relayRows.length;
        const buttonCount = Number.isInteger(d.button_count) ? Math.max(Number(d.button_count), 0) : 0;
        const hasInputMapping = buttonCount > 0 || (Array.isArray(d.input_config) && d.input_config.length > 0);
        const capabilitySummary = asCapabilitySummaryList(d.capabilities);
        const hasIrTx = capabilitySummary.some((item) => item.enabled && item.key === "ir_tx");
        const hasIrRx = capabilitySummary.some((item) => item.enabled && item.key === "ir_rx");
        const ioSummaryParts = [];
        if (relayCount > 0) {
          ioSummaryParts.push(`relays: ${relayCount}`);
        }
        if (buttonCount > 0) {
          ioSummaryParts.push(`inputs: ${buttonCount}`);
        }
        if (irCapable) {
          if (hasIrTx && hasIrRx) {
            ioSummaryParts.push("ir: tx/rx");
          } else if (hasIrTx) {
            ioSummaryParts.push("ir: tx");
          } else if (hasIrRx) {
            ioSummaryParts.push("ir: rx");
          } else {
            ioSummaryParts.push("ir: enabled");
          }
        }
        const ioSummaryLabel = ioSummaryParts.length > 0 ? ioSummaryParts.join(" | ") : "no relay/input channels";
        const ioSummaryMeta =
          relayCount > 0
            ? `power_restore: ${d.power_restore_mode || "-"}`
            : irCapable
              ? "infrared control available"
              : "power_restore: -";
        const mainControlSections = [];

        if (showRelayControls) {
          mainControlSections.push(`
            <section class="device-block">
              <div class="device-block-title">Relay Controls</div>
              <div class="relay-grid">
                ${relayControls || '<div class="muted">No relay channels reported.</div>'}
              </div>
              <div class="device-action-row">
                <button data-relay-all="${escapeHtml(d.id)}:on">All ON</button>
                <button data-relay-all="${escapeHtml(d.id)}:off" class="secondary">All OFF</button>
              </div>
            </section>
          `);
        }

        if (irCapable) {
          mainControlSections.push(renderIrControls(d, isOwner));
        }

        if (hasInputMapping) {
          mainControlSections.push(`
            <section class="device-block">
              <div class="device-block-title">Input Mapping</div>
              ${buttonStates}
              ${admin ? inputEditor : '<div class="muted">Input mapping edit is admin only.</div>'}
              ${admin ? `
              <div class="device-action-row">
                <button data-save-io="${escapeHtml(d.id)}" class="secondary">Save I/O</button>
              </div>` : ""}
            </section>
          `);
        }

        mainControlSections.push(`
          <section class="device-block">
            <div class="device-block-title">Connectivity Mode</div>
            ${connectivityEditor}
            <div class="device-action-row">
              <button data-save-conn="${escapeHtml(d.id)}" class="secondary" ${(admin || d.is_owner) ? "" : "disabled"}>Save Connectivity</button>
            </div>
          </section>
        `);

        const wsOnline = typeof d.ws_online === "boolean" ? d.ws_online : null;
        const online = wsOnline === null
          ? Boolean(d.is_active) && isLikelyOnline(d.last_seen_at)
          : wsOnline;
        const statusBadge = !d.is_active
          ? '<span class="pill bad">inactive</span>'
          : online
            ? '<span class="pill ok">online</span>'
          : '<span class="pill warn">stale/offline</span>';
        const ownershipBadge = d.owner_user_id
          ? '<span class="pill ok">claimed</span>'
          : '<span class="pill warn">unclaimed</span>';

        return `
          <article class="device-card ${isCollapsed ? "is-collapsed" : ""}" data-device-id="${escapeHtml(d.id)}">
            <div class="device-card-head">
              <div>
                <h3 class="device-title">${escapeHtml(d.name)}</h3>
                <div class="device-head-badges">
                  ${statusBadge}
                  ${ownershipBadge}
                  <span class="pill">${escapeHtml(d.device_class || "-")}</span>
                </div>
                <div class="meta code">${escapeHtml(d.device_uid)}</div>
                <div class="meta">model: ${escapeHtml(d.model || "-")}</div>
              </div>
              <div class="device-head-actions">
                <button type="button" class="collapse-toggle" data-device-collapse="${escapeHtml(cardKey)}">${isCollapsed ? "Expand" : "Collapse"}</button>
              </div>
            </div>

            <div class="device-card-body" ${isCollapsed ? "hidden" : ""}>
              <div class="device-meta-grid">
                <div class="device-meta-card">
                  <div class="device-meta-label">Ownership</div>
                  <div class="device-meta-value">${escapeHtml(d.owner_email || "unclaimed")}</div>
                  <div class="meta code">owner_id: ${escapeHtml(d.owner_user_id || "-")}</div>
                  <div class="meta code">claim_code: ${escapeHtml(d.claim_code || "-")}</div>
                </div>
                <div class="device-meta-card">
                  <div class="device-meta-label">Firmware</div>
                  <div class="device-meta-value">fw: ${escapeHtml(d.firmware_version || "-")}</div>
                  <div class="meta">ota: ${escapeHtml(d.ota_channel || "-")}/${escapeHtml(d.ota_security_version || "-")}</div>
                </div>
                <div class="device-meta-card">
                  <div class="device-meta-label">Connectivity</div>
                  <div class="device-meta-value">last_seen: ${escapeHtml(formatIsoWithOffset(d.last_seen_at))}</div>
                  <div class="meta">age: ${escapeHtml(ageLabel(d.last_seen_at))}</div>
                  <div class="meta">ip: ${escapeHtml(d.last_ip || "-")}</div>
                  <div class="meta">mode: ${escapeHtml(connectivityConfig.mode)}</div>
                  <div class="meta">broker: ${escapeHtml(connectivityConfig.mqtt.host || "-")}:${escapeHtml(connectivityConfig.mqtt.port)}</div>
                </div>
                <div class="device-meta-card">
                  <div class="device-meta-label">I/O Summary</div>
                  <div class="device-meta-value">${escapeHtml(ioSummaryLabel)}</div>
                  <div class="meta">${escapeHtml(ioSummaryMeta)}</div>
                </div>
              </div>

              <div class="device-main-grid">
                ${mainControlSections.join("")}
              </div>

              <div class="device-footer-grid">
                <section class="device-block">
                  <div class="device-block-title">Recent Device Events</div>
                  <div class="device-event">
                    <div><strong>Last action</strong></div>
                    <div>${escapeHtml(summarizeLastAction(d))}</div>
                    <div><strong>Last input</strong></div>
                    <div>${escapeHtml(summarizeLastInputEvent(d))}</div>
                  </div>
                </section>

                ${admin ? renderAutomationTesting(d) : ""}

                <section class="device-block">
                  <div class="device-block-title">Device Actions</div>
                  <div class="device-action-row">
                    <button data-rotate-token="${escapeHtml(d.id)}" class="warn">Rotate Token</button>
                    <button data-release-device="${escapeHtml(d.id)}" class="danger">Release</button>
                    <button data-delete-device="${escapeHtml(d.id)}" class="danger">Delete</button>
                    ${d.claim_code ? `<button data-claim-code="${escapeHtml(d.claim_code)}" class="secondary">Claim This Code</button>` : ""}
                  </div>
                  <div class="device-action-row">
                    <button data-device-reboot="${escapeHtml(d.id)}" class="warn" ${isOwner ? "" : "disabled"}>Reboot Device</button>
                    <button data-device-factory-reset="${escapeHtml(d.id)}" class="danger" ${isOwner ? "" : "disabled"}>Factory Reset</button>
                  </div>
                  <div class="device-action-row">
                    <button data-ota-check="${escapeHtml(d.id)}" class="secondary" ${isOwner ? "" : "disabled"}>Check OTA</button>
                    <button data-ota-install="${escapeHtml(d.id)}" class="warn" ${isOwner ? "" : "disabled"}>Install OTA</button>
                  </div>
                  <div class="grid cols-2">
                    <div>
                      <label>Wi-Fi SSID</label>
                      <input data-wifi-ssid placeholder="example-wifi-name" />
                    </div>
                    <div>
                      <label>Wi-Fi Password</label>
                      <input data-wifi-password type="password" placeholder="optional for open network" />
                    </div>
                  </div>
                  <label class="io-inline-check">
                    <input type="checkbox" data-wifi-reboot checked />
                    Reboot device after Wi-Fi update
                  </label>
                  <div class="device-action-row">
                    <button data-wifi-set="${escapeHtml(d.id)}" class="secondary" ${isOwner ? "" : "disabled"}>Set Wi-Fi</button>
                    <button data-wifi-clear="${escapeHtml(d.id)}" class="warn" ${isOwner ? "" : "disabled"}>Clear Wi-Fi</button>
                  </div>
                  <div class="meta">
                    ${isOwner
                      ? "Wi-Fi commands use realtime channel and require the device to be online."
                      : "Only owner/admin can send Wi-Fi set/clear commands."}
                  </div>
                </section>
              </div>
            </div>
          </article>
        `;
      }).join("");

      $("devicesTable").innerHTML = `<div class="devices-list">${cards}</div>`;
      if (isAdminUser()) {
        for (const device of visibleDevices) {
          if (device && typeof device.id === "string") {
            syncAutomationTestingControls(device.id);
          }
        }
      }
    }

    function renderSigningKeys(keys) {
      $("signingKeysTable").innerHTML = `
        <table>
          <thead><tr><th>Key ID</th><th>Status</th><th>Created</th><th>Rotated</th></tr></thead>
          <tbody>
            ${keys.map((k) => `<tr><td class="code">${k.key_id}</td><td>${k.status}</td><td>${escapeHtml(formatIsoWithOffset(k.created_at))}</td><td>${escapeHtml(formatIsoWithOffset(k.rotated_at))}</td></tr>`).join("")}
          </tbody>
        </table>
      `;
    }

    function renderReleases(releases) {
      state.releases = releases;
      $("releasesTable").innerHTML = `
        <table>
          <thead><tr><th>Version</th><th>Security</th><th>Channel</th><th>Key IDs</th><th>Expires</th><th>Actions</th></tr></thead>
          <tbody>
            ${releases.map((r) => `
              <tr>
                <td>${r.version}</td>
                <td>${r.security_version}</td>
                <td>${r.channel}</td>
                <td><div class="code">active:${r.verification_key_id || "-"}<br/>next:${r.next_verification_key_id || "-"}</div></td>
                <td>${escapeHtml(formatIsoWithOffset(r.expires_at))}</td>
                <td>
                  <button data-verify-release="${r.id}" class="secondary">Verify</button>
                  <button data-delete-release="${r.id}" class="danger">Delete</button>
                </td>
              </tr>
            `).join("")}
          </tbody>
        </table>
      `;
    }

    function renderBackupRuns(runs) {
      $("backupRunsTable").innerHTML = `
        <table>
          <thead><tr><th>Operation</th><th>Status</th><th>Started</th><th>Finished</th><th>Backup Path</th><th>Error</th></tr></thead>
          <tbody>
            ${runs.map((r) => `
              <tr>
                <td>${r.operation}</td>
                <td>${r.status}</td>
                <td>${escapeHtml(formatIsoWithOffset(r.started_at))}</td>
                <td>${escapeHtml(formatIsoWithOffset(r.finished_at))}</td>
                <td class="code">${r.backup_path || "-"}</td>
                <td>${r.error_message || "-"}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
      `;
    }

    function renderAudit(rows) {
      $("auditTable").innerHTML = `
        <table>
          <thead><tr><th>Time</th><th>Source</th><th>Action</th><th>Device</th><th>User</th><th>Details</th></tr></thead>
          <tbody>
            ${rows.map((r) => `
              <tr>
                <td>${escapeHtml(formatIsoWithOffset(r.created_at))}</td>
                <td>${r.source || "-"}</td>
                <td>${r.action}</td>
                <td>${r.device_uid || "-"}</td>
                <td>${r.user_email || "-"}</td>
                <td class="code">${JSON.stringify(r.details || {})}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
      `;
    }

    async function loadOverview() {
      if (!isAdminUser()) {
        return;
      }
      const overview = await api("/api/v1/admin/overview", { method: "GET" });
      renderKpis(overview);
    }

    async function loadUsers() {
      if (!isAdminUser()) {
        state.users = [];
        renderUsers([]);
        return;
      }
      const users = await api("/api/v1/admin/users", { method: "GET" });
      state.users = users;
      renderUsers(users);
    }

    async function loadDevices() {
      const path = isAdminUser() ? "/api/v1/admin/devices" : "/api/v1/devices";
      const devices = await api(path, { method: "GET" });

      const previousStatesByUid = new Map();
      const previousOwnerByDeviceId = new Map();
      for (const device of state.devices || []) {
        if (!device || typeof device.device_uid !== "string") {
          continue;
        }
        if (typeof device.id === "string") {
          previousOwnerByDeviceId.set(device.id, typeof device.owner_user_id === "string" ? device.owner_user_id : "");
        }
        if (!device.button_states || typeof device.button_states !== "object" || Array.isArray(device.button_states)) {
          continue;
        }
        previousStatesByUid.set(device.device_uid, { ...device.button_states });
      }

      for (const device of devices) {
        if (!device || typeof device.device_uid !== "string") {
          continue;
        }
        const cachedStates = previousStatesByUid.get(device.device_uid);
        if (cachedStates) {
          device.button_states = { ...cachedStates };
        }
        initializeDeviceButtonStates(device);

        const ownerUserId = typeof device.owner_user_id === "string" ? device.owner_user_id : "";
        const previousOwnerUserId = previousOwnerByDeviceId.get(device.id) || "";
        if (!ownerUserId || ownerUserId !== previousOwnerUserId) {
          state.automationTestingByDevice.delete(device.id);
        }
      }

      const knownDeviceIds = new Set(
        devices
          .map((device) => (device && typeof device.id === "string" ? device.id : ""))
          .filter((id) => id.length > 0)
      );
      for (const cachedDeviceId of state.automationTestingByDevice.keys()) {
        if (!knownDeviceIds.has(cachedDeviceId)) {
          state.automationTestingByDevice.delete(cachedDeviceId);
        }
      }
      for (const cachedDeviceId of state.irCodesByDevice.keys()) {
        if (!knownDeviceIds.has(cachedDeviceId)) {
          state.irCodesByDevice.delete(cachedDeviceId);
        }
      }
      for (const cachedDeviceId of state.irLastResultByDevice.keys()) {
        if (!knownDeviceIds.has(cachedDeviceId)) {
          state.irLastResultByDevice.delete(cachedDeviceId);
        }
      }

      state.devices = devices;
      renderDevices(devices);

      for (const device of devices) {
        if (!device || typeof device.id !== "string") {
          continue;
        }
        if (!isIrCapableDevice(device) || !currentUserOwnsDevice(device)) {
          continue;
        }
        const cache = getIrCodeCache(device.id);
        if (cache.loading || cache.loadedAt || cache.codes.length > 0 || cache.error) {
          continue;
        }
        void loadIrCodesForDevice(device.id, { force: true }).catch((error) => {
          const message = error instanceof Error ? error.message : "ir_code_load_failed";
          log(`IR code preload failed: ${device.id} ${message}`);
        });
      }
    }

    async function loadOta() {
      if (!isAdminUser()) {
        return;
      }
      const [keys, releases] = await Promise.all([
        api("/api/v1/ota/signing-keys", { method: "GET" }),
        api("/api/v1/ota/releases", { method: "GET" })
      ]);
      renderSigningKeys(keys);
      renderReleases(releases);
    }

    async function loadOpsRuns() {
      if (!isAdminUser()) {
        return;
      }
      const runs = await api("/api/v1/admin/ops/backup/runs?limit=50", { method: "GET" });
      renderBackupRuns(runs);
    }

    async function loadAudit() {
      if (!isAdminUser()) {
        return;
      }
      const source = encodeURIComponent($("auditFilterSource").value || "");
      const action = encodeURIComponent($("auditFilterAction").value || "");
      const rows = await api(`/api/v1/admin/audit?limit=100&source=${source}&action=${action}`, { method: "GET" });
      renderAudit(rows);
    }

    async function loadMetrics() {
      const response = await fetch(baseUrl() + "/metrics");
      $("metricsOutput").textContent = await response.text();
    }

    async function loadAll() {
      if (isAdminUser()) {
        await Promise.all([
          loadOverview(),
          loadUsers(),
          loadDevices(),
          loadOta(),
          loadOpsRuns(),
          loadAudit(),
          loadMetrics()
        ]);
      } else {
        await Promise.all([
          loadDevices(),
          loadMetrics()
        ]);
      }
      log("Dashboard data refreshed.");
    }

    async function claimDeviceByCode(claimCode) {
      if (!claimCode || !claimCode.trim()) {
        throw new Error("Claim code is required.");
      }
      const out = await api("/api/v1/devices/claim", {
        method: "POST",
        body: JSON.stringify({ claim_code: claimCode.trim() })
      });
      $("claimCodeInput").value = "";
      log(`Claimed device: ${out.device?.device_uid || "ok"}`);
      if (isAdminUser()) {
        await Promise.all([loadDevices(), loadOverview(), loadUsers()]);
      } else {
        await loadDevices();
      }
    }

    async function login(endpoint) {
      const body = {
        email: $("email").value.trim(),
        password: $("password").value
      };
      if (!body.email || !body.password) {
        throw new Error("Email and password are required.");
      }

      const payload = endpoint === "register"
        ? { ...body, name: body.email.split("@")[0] || "admin" }
        : body;

      const out = await api(`/api/v1/auth/${endpoint}`, {
        method: "POST",
        body: JSON.stringify(payload)
      });

      state.user = out.user;
      state.accessToken = out.access_token;
      state.refreshToken = out.refresh_token;
      setAuthState();
      persistSession();
      log(`${endpoint} successful for ${state.user.email}.`);
      connectRealtime(true);
      await loadPreferences();
      await loadAll();
    }

    async function refreshToken() {
      await refreshAccessToken();
      connectRealtime();
    }

    async function logout() {
      if (state.refreshToken) {
        try {
          await rawRequest("/api/v1/auth/logout", {
            method: "POST",
            body: JSON.stringify({ refresh_token: state.refreshToken })
          });
        } catch {
          // Continue local logout even if revoke fails.
        }
      }
      stopRealtime({ silent: true });
      clearSession();
      setAuthState();
      log("Logged out.");
    }

    $("loginBtn").addEventListener("click", async () => {
      try { await login("login"); } catch (error) { log(`Login failed: ${error.message}`); }
    });
    $("registerBtn").addEventListener("click", async () => {
      try { await login("register"); } catch (error) { log(`Register failed: ${error.message}`); }
    });
    $("refreshBtn").addEventListener("click", async () => {
      try { await refreshToken(); } catch (error) { log(`Refresh failed: ${error.message}`); }
    });
    $("loadPrefsBtn").addEventListener("click", async () => {
      try { await loadPreferences(); } catch (error) { log(`Load prefs failed: ${error.message}`); }
    });
    $("savePrefsBtn").addEventListener("click", async () => {
      try { await savePreferences(); } catch (error) { log(`Save prefs failed: ${error.message}`); }
    });
    $("logoutBtn").addEventListener("click", async () => {
      try { await logout(); } catch (error) { log(`Logout failed: ${error.message}`); }
    });

    $("reloadAllBtn").addEventListener("click", async () => {
      try { await loadAll(); } catch (error) { log(`Reload failed: ${error.message}`); }
    });

    $("refreshDevicesBtn").addEventListener("click", async () => {
      try { await loadDevices(); log("Devices refreshed."); } catch (error) { log(`Devices refresh failed: ${error.message}`); }
    });

    $("devicesSearchInput").addEventListener("input", () => {
      state.deviceSearchQuery = $("devicesSearchInput").value || "";
      renderDevices(state.devices);
    });

    $("claimDeviceBtn").addEventListener("click", async () => {
      try { await claimDeviceByCode($("claimCodeInput").value || ""); } catch (error) { log(`Claim failed: ${error.message}`); }
    });
    $("baseUrl").addEventListener("change", () => {
      persistSession();
      connectRealtime(true);
    });

    $("loadMetricsBtn").addEventListener("click", async () => {
      try { await loadMetrics(); log("Loaded /metrics."); } catch (error) { log(`Metrics load failed: ${error.message}`); }
    });

    $("runBackupBtn").addEventListener("click", async () => {
      try {
        const out = await api("/api/v1/admin/ops/backup/run", { method: "POST", body: "{}" });
        log(`Backup completed: ${out.backup_path}`);
        await loadOpsRuns();
      } catch (error) {
        log(`Backup failed: ${error.message}`);
      }
    });

    $("runRestoreDrillBtn").addEventListener("click", async () => {
      try {
        const backupPath = $("restorePath").value.trim();
        const out = await api("/api/v1/admin/ops/restore-drill/run", {
          method: "POST",
          body: JSON.stringify({ backup_path: backupPath || undefined })
        });
        log(`Restore drill completed in ${out.elapsed_ms} ms.`);
        await loadOpsRuns();
      } catch (error) {
        log(`Restore drill failed: ${error.message}`);
      }
    });

    $("simulateAlertsBtn").addEventListener("click", async () => {
      try {
        const out = await api("/api/v1/admin/ops/alerts/simulate", {
          method: "POST",
          body: JSON.stringify({})
        });
        const fired = (out.alerts || []).filter((x) => x.fired).length;
        log(`Alert simulation done. Fired alerts: ${fired}.`);
      } catch (error) {
        log(`Alert simulation failed: ${error.message}`);
      }
    });

    $("clearLogBtn").addEventListener("click", () => { $("log").textContent = ""; });

    $("createSigningKeyBtn").addEventListener("click", async () => {
      try {
        await api("/api/v1/ota/signing-keys", {
          method: "POST",
          body: JSON.stringify({
            key_id: $("signingKeyId").value.trim(),
            status: $("signingKeyStatus").value,
            private_key_secret_ref: $("signingSecretRef").value.trim(),
            public_key_pem: $("signingPublicPem").value
          })
        });
        log("Signing key created.");
        await loadOta();
      } catch (error) {
        log(`Create signing key failed: ${error.message}`);
      }
    });

    $("rotateSigningKeysBtn").addEventListener("click", async () => {
      try {
        const out = await api("/api/v1/ota/signing-keys/rotate", { method: "POST", body: "{}" });
        log(`Signing keys rotated. New active: ${out.active_key?.key_id || "n/a"}`);
        await loadOta();
      } catch (error) {
        log(`Rotate signing keys failed: ${error.message}`);
      }
    });

    $("uploadReleaseBtn").addEventListener("click", async () => {
      const fileInput = $("releaseBinFile");
      const firmwareFile = fileInput?.files?.[0];
      if (!firmwareFile) {
        log("Select a .bin firmware file first.");
        return;
      }

      const metadataText = $("releaseMetadataJson").value.trim();
      if (metadataText) {
        try {
          const parsed = JSON.parse(metadataText);
          if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
            throw new Error("invalid");
          }
        } catch {
          log("Metadata must be a valid JSON object.");
          return;
        }
      }

      try {
        const form = new FormData();

        const model = $("releaseModel").value.trim();
        const version = $("releaseVersion").value.trim();
        const channel = $("releaseChannel").value.trim();
        const expiresAt = $("releaseExpiresAt").value.trim();
        const securityVersion = $("releaseSecurityVersion").value.trim();

        if (model) form.append("model", model);
        if (version) form.append("version", version);
        if (channel) form.append("channel", channel);
        if (expiresAt) form.append("expires_at", expiresAt);
        if (securityVersion) form.append("security_version", securityVersion);
        form.append("is_active", "true");
        form.append("auto_sign", "true");
        form.append("metadata", metadataText || JSON.stringify({ created_from: "dashboard_upload" }));
        form.append("firmware", firmwareFile, firmwareFile.name);

        const out = await api("/api/v1/ota/releases/upload", {
          method: "POST",
          body: form
        });

        log(
          `OTA upload created release ${out.model} ${out.version} (${out.channel}) sha=${String(
            out.sha256 || ""
          ).slice(0, 12)}...`
        );
        await loadOta();
      } catch (error) {
        log(`Upload release failed: ${error.message}`);
      }
    });

    $("releasesTable").addEventListener("click", async (event) => {
      const target = event.target;
      if (!(target instanceof HTMLElement)) return;
      const verifyId = target.getAttribute("data-verify-release");
      if (verifyId) {
        try {
          const out = await api(`/api/v1/ota/releases/${encodeURIComponent(verifyId)}/verify`, { method: "GET" });
          log(`Release ${verifyId} verification: ${out.ok ? "OK" : `FAILED (${out.reason})`}`);
        } catch (error) {
          log(`Release verify failed: ${error.message}`);
        }
        return;
      }

      const deleteId = target.getAttribute("data-delete-release");
      if (deleteId) {
        if (!window.confirm(`Delete OTA release ${deleteId}? This cannot be undone.`)) {
          return;
        }
        try {
          await api(`/api/v1/ota/releases/${encodeURIComponent(deleteId)}`, { method: "DELETE" });
          log(`OTA release ${deleteId} deleted.`);
          await loadOta();
        } catch (error) {
          log(`Release delete failed: ${error.message}`);
        }
      }
    });

    $("usersTable").addEventListener("click", async (event) => {
      const target = event.target;
      if (!(target instanceof HTMLElement)) return;
      const userDeleteId = target.getAttribute("data-user-delete");
      const userId = target.getAttribute("data-user-save");

      try {
        if (userDeleteId) {
          if (!window.confirm(`Delete user ${userDeleteId}? This action cannot be undone.`)) {
            return;
          }
          await api(`/api/v1/admin/users/${encodeURIComponent(userDeleteId)}`, {
            method: "DELETE"
          });
          log(`User ${userDeleteId} deleted.`);
          await Promise.all([loadUsers(), loadDevices(), loadOverview()]);
          return;
        }

        if (!userId) return;

        const name = document.querySelector(`[data-user-name=\"${userId}\"]`)?.value || "";
        const role = document.querySelector(`[data-user-role=\"${userId}\"]`)?.value || "user";
        const isActive = Boolean(document.querySelector(`[data-user-active=\"${userId}\"]`)?.checked);
        await api(`/api/v1/admin/users/${encodeURIComponent(userId)}`, {
          method: "PATCH",
          body: JSON.stringify({ name, role, is_active: isActive })
        });
        log(`User ${userId} updated.`);
        await loadUsers();
      } catch (error) {
        log(`User update failed: ${error.message}`);
      }
    });

    $("devicesTable").addEventListener("click", async (event) => {
      const target = event.target;
      if (!(target instanceof Element)) return;

      const getActionAttr = (name) => {
        const el = target.closest(`[${name}]`);
        return el instanceof Element ? el.getAttribute(name) : null;
      };

      const collapseEl = target.closest("[data-device-collapse]");
      const deviceCollapse = collapseEl instanceof Element ? collapseEl.getAttribute("data-device-collapse") : null;
      const relayCmd = getActionAttr("data-relay");
      const relayAll = getActionAttr("data-relay-all");
      const saveIo = getActionAttr("data-save-io");
      const saveConn = getActionAttr("data-save-conn");
      const rotateToken = getActionAttr("data-rotate-token");
      const releaseDevice = getActionAttr("data-release-device");
      const deleteDevice = getActionAttr("data-delete-device");
      const claimCode = getActionAttr("data-claim-code");
      const deviceReboot = getActionAttr("data-device-reboot");
      const deviceFactoryReset = getActionAttr("data-device-factory-reset");
      const otaCheck = getActionAttr("data-ota-check");
      const otaInstall = getActionAttr("data-ota-install");
      const wifiSet = getActionAttr("data-wifi-set");
      const wifiClear = getActionAttr("data-wifi-clear");
      const irAction = getActionAttr("data-ir-action");
      const irDeviceId = getActionAttr("data-ir-device");
      const loadAutomationTests = getActionAttr("data-load-automation-tests");
      const runAutomationNow = getActionAttr("data-run-automation-now");
      const deleteAutomation = getActionAttr("data-delete-automation");
      const runScheduleNow = getActionAttr("data-run-schedule-now");
      const createAutomation = getActionAttr("data-create-automation");
      const createSchedule = getActionAttr("data-create-schedule");

      try {
        if (deviceCollapse) {
          const nextCollapsed = !Boolean(state.collapsedDeviceCards[deviceCollapse]);
          state.collapsedDeviceCards[deviceCollapse] = nextCollapsed;
          persistCollapsedDeviceCards();
          const card = collapseEl instanceof Element ? collapseEl.closest(".device-card") : null;
          if (card instanceof HTMLElement) {
            const body = card.querySelector(".device-card-body");
            if (body instanceof HTMLElement) {
              body.hidden = nextCollapsed;
            }
            card.classList.toggle("is-collapsed", nextCollapsed);
          }
          if (collapseEl instanceof HTMLElement) {
            collapseEl.textContent = nextCollapsed ? "Expand" : "Collapse";
          }
          return;
        }

        if (loadAutomationTests) {
          if (!isAdminUser()) {
            throw new Error("forbidden: Insufficient role.");
          }
          await withDeviceActionLock(loadAutomationTests, async () => {
            const loaded = await loadAutomationTesting(loadAutomationTests, { force: true });
            log(
              `Automation testing loaded: ${loadAutomationTests} automations=${loaded.automations.length} schedules=${loaded.schedules.length}`
            );
          });
          return;
        }

        if (createAutomation) {
          if (!isAdminUser()) {
            throw new Error("forbidden: Insufficient role.");
          }
          const deviceId = createAutomation;
          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device || !device.owner_user_id) {
            throw new Error("Device must be claimed before creating automation.");
          }
          const card = target.closest(".device-card");
          if (!(card instanceof HTMLElement)) {
            throw new Error("Device card context not found.");
          }

          await withDeviceActionLock(deviceId, async () => {
            const payload = collectAutomationCreatePayload(device, card);
            const out = await api(`/api/v1/admin/devices/${encodeURIComponent(deviceId)}/automations`, {
              method: "POST",
              body: JSON.stringify(payload)
            });
            log(`Automation created: ${deviceId} id=${out.id} trigger=${out.trigger_type} action=${out.action_type}`);
            await loadAutomationTesting(deviceId, { force: true });
          });
          return;
        }

        if (createSchedule) {
          if (!isAdminUser()) {
            throw new Error("forbidden: Insufficient role.");
          }
          const deviceId = createSchedule;
          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device || !device.owner_user_id) {
            throw new Error("Device must be claimed before creating schedule.");
          }
          const card = target.closest(".device-card");
          if (!(card instanceof HTMLElement)) {
            throw new Error("Device card context not found.");
          }

          await withDeviceActionLock(deviceId, async () => {
            const payload = collectScheduleCreatePayload(device, card);
            const out = await api(`/api/v1/admin/devices/${encodeURIComponent(deviceId)}/schedules`, {
              method: "POST",
              body: JSON.stringify(payload)
            });
            log(`Schedule created: ${deviceId} id=${out.id} type=${out.schedule_type} action=${out.action}`);
            await loadAutomationTesting(deviceId, { force: true });
          });
          return;
        }

        if (runAutomationNow) {
          if (!isAdminUser()) {
            throw new Error("forbidden: Insufficient role.");
          }
          const split = runAutomationNow.split(":");
          const deviceId = split[0] || "";
          const automationId = split[1] || "";
          if (!deviceId || !automationId) {
            throw new Error("Invalid automation run-now payload.");
          }

          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device || !device.owner_user_id) {
            throw new Error("Device must be claimed before run-now.");
          }

          await withDeviceActionLock(deviceId, async () => {
            const started = Date.now();
            const out = await api(`/api/v1/admin/automations/${encodeURIComponent(automationId)}/run-now`, {
              method: "POST",
              body: "{}"
            });
            const latencyMs = Date.now() - started;
            const status = typeof out.status === "string" ? out.status : "unknown";
            const reason = typeof out.reason === "string" ? out.reason : "";
            const code = typeof out.code === "string" ? out.code : "";
            log(
              `Automation run-now: device=${deviceId} automation=${automationId} status=${status} latency=${latencyMs}ms` +
              `${reason ? ` reason=${reason}` : ""}${code ? ` code=${code}` : ""}`
            );
            await loadDevices();
            await loadAutomationTesting(deviceId, { force: true });
          });
          return;
        }

        if (deleteAutomation) {
          if (!isAdminUser()) {
            throw new Error("forbidden: Insufficient role.");
          }
          const split = deleteAutomation.split(":");
          const deviceId = split[0] || "";
          const automationId = split[1] || "";
          if (!deviceId || !automationId) {
            throw new Error("Invalid automation delete payload.");
          }

          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device || !device.owner_user_id) {
            throw new Error("Device must be claimed before deleting automation.");
          }

          if (!window.confirm(`Delete automation ${automationId}? This action cannot be undone.`)) {
            return;
          }

          await withDeviceActionLock(deviceId, async () => {
            await api(`/api/v1/admin/automations/${encodeURIComponent(automationId)}`, {
              method: "DELETE"
            });
            log(`Automation deleted: device=${deviceId} automation=${automationId}`);
            await loadAutomationTesting(deviceId, { force: true });
          });
          return;
        }

        if (runScheduleNow) {
          if (!isAdminUser()) {
            throw new Error("forbidden: Insufficient role.");
          }
          const split = runScheduleNow.split(":");
          const deviceId = split[0] || "";
          const scheduleId = split[1] || "";
          if (!deviceId || !scheduleId) {
            throw new Error("Invalid schedule run-now payload.");
          }

          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device || !device.owner_user_id) {
            throw new Error("Device must be claimed before run-now.");
          }

          await withDeviceActionLock(deviceId, async () => {
            const started = Date.now();
            const out = await api(`/api/v1/admin/schedules/${encodeURIComponent(scheduleId)}/run-now`, {
              method: "POST",
              body: "{}"
            });
            const latencyMs = Date.now() - started;
            const status = typeof out.status === "string" ? out.status : "unknown";
            const reason = typeof out.reason === "string" ? out.reason : "";
            const code = typeof out.code === "string" ? out.code : "";
            log(
              `Schedule run-now: device=${deviceId} schedule=${scheduleId} status=${status} latency=${latencyMs}ms` +
              `${reason ? ` reason=${reason}` : ""}${code ? ` code=${code}` : ""}`
            );
            await loadDevices();
            await loadAutomationTesting(deviceId, { force: true });
          });
          return;
        }

        if (irAction && irDeviceId) {
          const deviceId = irDeviceId;
          const action = irAction.trim().toLowerCase();
          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device) {
            throw new Error("Device not found in dashboard state.");
          }
          if (!isIrCapableDevice(device)) {
            throw new Error("Device is not IR capable.");
          }
          if (!currentUserOwnsDevice(device)) {
            throw new Error("forbidden: Only owner/admin can send IR commands.");
          }

          if (action === "load_codes") {
            await withDeviceActionLock(deviceId, async () => {
              const codes = await loadIrCodesForDevice(deviceId, { force: true });
              log(`IR code list refreshed: ${deviceId} count=${codes.length}`);
              setIrLastResult(deviceId, `Code list refreshed (${codes.length})`);
            });
            return;
          }

          const card = target.closest(".device-card") || findDeviceCardElement(deviceId);
          if (!(card instanceof HTMLElement)) {
            throw new Error("Device card context not found.");
          }

          captureIrFormFromCard(deviceId, card);
          const irForm = getIrFormState(deviceId);
          const learnType = irForm.learnType === "ac" ? "ac" : "button";
          const codeName = irForm.codeName.trim();
          const selectEl = card.querySelector("[data-ir-code-select]");
          const selectedCodeId = selectEl instanceof HTMLSelectElement ? selectEl.value.trim() : "";
          if (selectedCodeId) {
            setIrCodeSelection(deviceId, selectedCodeId);
          }
          const cache = getIrCodeCache(deviceId);
          const selectedCode = cache.codes.find((entry) => entry && entry.id === selectedCodeId) || null;
          const selectedCodeName =
            selectedCode && typeof selectedCode.code_name === "string" ? selectedCode.code_name : "";

          const commandPayload = {
            device_id: deviceId,
            scope: "ir",
            operation: action
          };

          if (action === "learn_start") {
            commandPayload.learn_type = learnType;
            if (codeName) {
              commandPayload.code_name = codeName;
            }
          } else if (action === "learn_test") {
            // no-op, command payload already contains operation
          } else if (action === "learn_save") {
            commandPayload.learn_type = learnType;
            if (codeName) {
              commandPayload.code_name = codeName;
            }
            const candidate = collectIrCandidateFromCard(card);
            if (candidate) {
              commandPayload.candidate = candidate;
            }
          } else if (action === "learn_discard") {
            // no-op, command payload already contains operation
          } else if (action === "send_code" || action === "delete_code") {
            if (selectedCodeId) {
              commandPayload.code_id = selectedCodeId;
            } else if (selectedCodeName) {
              commandPayload.code_name = selectedCodeName;
            } else if (codeName) {
              commandPayload.code_name = codeName;
            } else {
              throw new Error("Select a stored code or provide a code name.");
            }
          } else if (action === "recognize_candidate") {
            const candidate = collectIrCandidateFromCard(card);
            if (!candidate) {
              throw new Error("Candidate protocol and payload are required for recognize.");
            }
            commandPayload.candidate = candidate;
            const topN = collectIrTopNFromCard(card);
            if (typeof topN === "number") {
              commandPayload.top_n = topN;
            }
          } else {
            throw new Error(`Unsupported IR action: ${action}`);
          }

          await withDeviceActionLock(deviceId, async () => {
            const result = await sendRealtimeCommand(commandPayload, { timeoutMs: 22000 });
            if (action === "recognize_candidate") {
              const matches = Array.isArray(result?.matches) ? result.matches : [];
              const top = matches[0] && typeof matches[0] === "object" ? matches[0] : null;
              const topLabel = top
                ? `${top.code_name || top.code_id || top.library_record_id || "unknown"}${
                  Number.isFinite(Number(top.score)) ? ` score=${Number(top.score).toFixed(3)}` : ""
                }`
                : "none";
              log(`IR recognize: ${deviceId} matches=${matches.length} top=${topLabel}`);
              setIrLastResult(deviceId, `Recognize matches=${matches.length} top=${topLabel}`);
              return;
            }

            const resultCodeName = typeof result?.code_name === "string" ? result.code_name : "";
            const resultCodeId = typeof result?.code_id === "string" ? result.code_id : "";
            log(
              `IR command sent: ${deviceId} ${action}` +
              `${resultCodeName ? ` code=${resultCodeName}` : resultCodeId ? ` code_id=${resultCodeId}` : ""}`
            );
            setIrLastResult(
              deviceId,
              `${action}${resultCodeName ? ` code=${resultCodeName}` : resultCodeId ? ` code_id=${resultCodeId}` : ""}`
            );

            if (action === "learn_save" || action === "delete_code") {
              await loadIrCodesForDevice(deviceId, { force: true });
            }
          });
          return;
        }

        if (wifiSet || wifiClear) {
          const deviceId = wifiSet || wifiClear || "";
          if (!deviceId) {
            throw new Error("Missing device id for Wi-Fi command.");
          }

          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device) {
            throw new Error("Device not found in dashboard state.");
          }
          if (!currentUserOwnsDevice(device)) {
            throw new Error("forbidden: Only owner/admin can send Wi-Fi commands.");
          }

          const card = target.closest(".device-card");
          const rebootEl = card ? card.querySelector("[data-wifi-reboot]") : null;
          const reboot = rebootEl instanceof HTMLInputElement ? rebootEl.checked : true;

          const wifiPayload = wifiSet
            ? {
                op: "set",
                reboot
              }
            : {
                op: "clear",
                reboot
              };

          if (wifiSet) {
            const ssidEl = card ? card.querySelector("[data-wifi-ssid]") : null;
            const passwordEl = card ? card.querySelector("[data-wifi-password]") : null;
            const ssid = ssidEl instanceof HTMLInputElement ? ssidEl.value.trim() : "";
            const password = passwordEl instanceof HTMLInputElement ? passwordEl.value : "";
            if (!ssid) {
              throw new Error("Wi-Fi SSID is required for set.");
            }
            wifiPayload.ssid = ssid;
            wifiPayload.password = password;
          }

          await withDeviceActionLock(deviceId, async () => {
            await sendRealtimeCommand(
              {
                device_id: deviceId,
                scope: "wifi",
                wifi: wifiPayload
              },
              { timeoutMs: 32000 }
            );
          });

          if (wifiSet) {
            const passwordEl = card ? card.querySelector("[data-wifi-password]") : null;
            if (passwordEl instanceof HTMLInputElement) {
              passwordEl.value = "";
            }
            log(`Wi-Fi set command sent: ${deviceId}${reboot ? " (reboot=true)" : ""}`);
          } else {
            log(`Wi-Fi clear command sent: ${deviceId}${reboot ? " (reboot=true)" : ""}`);
          }
          return;
        }

        if (deviceReboot || deviceFactoryReset) {
          const deviceId = deviceReboot || deviceFactoryReset || "";
          if (!deviceId) {
            throw new Error("Missing device id for device control command.");
          }
          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device) {
            throw new Error("Device not found in dashboard state.");
          }
          if (!currentUserOwnsDevice(device)) {
            throw new Error("forbidden: Only owner/admin can send device control commands.");
          }

          const operation = deviceFactoryReset ? "factory_reset" : "reboot";
          if (operation === "factory_reset") {
            const proceed = window.confirm(
              `Factory reset ${device.name || deviceId}? This clears device credentials/config and restarts it.`
            );
            if (!proceed) {
              return;
            }
          }

          await withDeviceActionLock(deviceId, async () => {
            await sendRealtimeCommand(
              {
                device_id: deviceId,
                scope: "device",
                operation
              },
              { timeoutMs: 14000 }
            );
          });
          log(`Device control command sent: ${deviceId} ${operation}`);
          return;
        }

        if (otaCheck || otaInstall) {
          const deviceId = otaCheck || otaInstall || "";
          if (!deviceId) {
            throw new Error("Missing device id for OTA command.");
          }
          const device = state.devices.find((item) => item && item.id === deviceId);
          if (!device) {
            throw new Error("Device not found in dashboard state.");
          }
          if (!currentUserOwnsDevice(device)) {
            throw new Error("forbidden: Only owner/admin can send OTA commands.");
          }

          const operation = otaInstall ? "install" : "check";
          const releaseChannelEl = $("releaseChannel");
          const releaseChannel =
            releaseChannelEl &&
            typeof releaseChannelEl.value === "string" &&
            /^(dev|beta|stable)$/i.test(releaseChannelEl.value.trim())
              ? releaseChannelEl.value.trim().toLowerCase()
              : "";
          const commandPayload = {
            device_id: deviceId,
            scope: "ota",
            operation
          };
          if (releaseChannel) {
            commandPayload.channel = releaseChannel;
          }

          await withDeviceActionLock(deviceId, async () => {
            await sendRealtimeCommand(commandPayload, { timeoutMs: 22000 });
          });
          log(`OTA command sent: ${deviceId} ${operation}${releaseChannel ? ` channel=${releaseChannel}` : ""}`);
          return;
        }

        if (relayCmd) {
          const [deviceId, idx, action] = relayCmd.split(":");
          const relayPath = isAdminUser()
            ? `/api/v1/admin/devices/${encodeURIComponent(deviceId)}/relays/${encodeURIComponent(idx)}`
            : `/api/v1/devices/${encodeURIComponent(deviceId)}/relays/${encodeURIComponent(idx)}`;
          await withDeviceActionLock(deviceId, async () => {
            await api(relayPath, {
              method: "POST",
              body: JSON.stringify({ action })
            });
          });
          log(`Relay command sent: ${deviceId} r${idx} ${action}`);
          await loadDevices();
          return;
        }

        if (relayAll) {
          const [deviceId, action] = relayAll.split(":");
          const relayAllPath = isAdminUser()
            ? `/api/v1/admin/devices/${encodeURIComponent(deviceId)}/relays/all`
            : `/api/v1/devices/${encodeURIComponent(deviceId)}/relays/all`;
          await withDeviceActionLock(deviceId, async () => {
            await api(relayAllPath, {
              method: "POST",
              body: JSON.stringify({ action })
            });
          });
          log(`All-relay command sent: ${deviceId} ${action}`);
          await loadDevices();
          return;
        }

        if (saveIo) {
          if (!isAdminUser()) {
            throw new Error("forbidden: Insufficient role.");
          }
          const device = state.devices.find((item) => item && item.id === saveIo);
          if (!device) {
            throw new Error("Device not found in dashboard state.");
          }

          const inputConfig = collectInputConfigFromEditor(device);
          await api(`/api/v1/admin/devices/${encodeURIComponent(saveIo)}`, {
            method: "PATCH",
            body: JSON.stringify({ input_config: inputConfig })
          });
          log(`I/O config saved: ${saveIo}`);
          await loadDevices();
          return;
        }

        if (saveConn) {
          const device = state.devices.find((item) => item && item.id === saveConn);
          if (!device) {
            throw new Error("Device not found in dashboard state.");
          }
          if (!isAdminUser() && !device.is_owner) {
            throw new Error("forbidden: Only owner/admin can edit connectivity.");
          }

          const connectivity = collectConnectivityConfigFromEditor(device);
          const mergedConfig = mergeConnectivityConfig(device, connectivity);
          const patchPath = isAdminUser()
            ? `/api/v1/admin/devices/${encodeURIComponent(saveConn)}`
            : `/api/v1/devices/${encodeURIComponent(saveConn)}`;
          await api(patchPath, {
            method: "PATCH",
            body: JSON.stringify({ config: mergedConfig })
          });
          log(`Connectivity config saved: ${saveConn} (${connectivity.mode})`);
          await loadDevices();
          return;
        }

        if (rotateToken) {
          const rotatePath = isAdminUser()
            ? `/api/v1/admin/devices/${encodeURIComponent(rotateToken)}/token/rotate`
            : `/api/v1/devices/${encodeURIComponent(rotateToken)}/token/rotate`;
          const out = await api(rotatePath, {
            method: "POST",
            body: "{}"
          });
          log(`Token rotated for ${rotateToken}: ${out.device_token}`);
          return;
        }

        if (releaseDevice) {
          const releasePath = isAdminUser()
            ? `/api/v1/admin/devices/${encodeURIComponent(releaseDevice)}/release`
            : `/api/v1/devices/${encodeURIComponent(releaseDevice)}/release`;
          const out = await api(releasePath, {
            method: "POST",
            body: "{}"
          });
          log(`Device ${releaseDevice} released. Claim code: ${out.claim_code}`);
          await Promise.all([loadDevices(), loadOverview()]);
          return;
        }

        if (deleteDevice) {
          if (!window.confirm(`Delete device ${deleteDevice}? This action cannot be undone.`)) {
            return;
          }
          await api(`/api/v1/devices/${encodeURIComponent(deleteDevice)}`, {
            method: "DELETE"
          });
          log(`Device deleted: ${deleteDevice}`);
          await Promise.all([loadDevices(), loadOverview(), loadUsers()]);
          return;
        }

        if (claimCode) {
          await claimDeviceByCode(claimCode);
          return;
        }
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        if (irAction && irDeviceId) {
          setIrLastResult(irDeviceId, `Error: ${errorMessage}`, "error");
        }
        log(`Device action failed: ${errorMessage}`);
      }
    });

    $("devicesTable").addEventListener("change", (event) => {
      const target = event.target;
      if (!(target instanceof HTMLElement)) return;
      syncIrFormStateFromElement(target);

      const irCodeSelectKey = target.getAttribute("data-ir-code-select");
      if (irCodeSelectKey) {
        const codeId = target instanceof HTMLSelectElement ? target.value : "";
        setIrCodeSelection(irCodeSelectKey, codeId);
        return;
      }

      const typeKey = target.getAttribute("data-io-type");
      if (typeKey) {
        syncInputRowControls(typeKey);
        return;
      }

      const linkedKey = target.getAttribute("data-io-linked");
      if (linkedKey) {
        syncInputRowControls(linkedKey);
        return;
      }

      const connModeKey = target.getAttribute("data-conn-mode");
      if (connModeKey) {
        syncConnectivityControls(connModeKey);
        return;
      }

      const automationTriggerKey = target.getAttribute("data-automation-trigger-type");
      if (automationTriggerKey) {
        syncAutomationTestingControls(automationTriggerKey);
        return;
      }

      const automationActionKey = target.getAttribute("data-automation-action-type");
      if (automationActionKey) {
        syncAutomationTestingControls(automationActionKey);
        return;
      }

      const scheduleTypeKey = target.getAttribute("data-schedule-type");
      if (scheduleTypeKey) {
        syncAutomationTestingControls(scheduleTypeKey);
        return;
      }

      const scheduleScopeKey = target.getAttribute("data-schedule-target-scope");
      if (scheduleScopeKey) {
        syncAutomationTestingControls(scheduleScopeKey);
      }
    });

    $("devicesTable").addEventListener("input", (event) => {
      const target = event.target;
      if (!(target instanceof HTMLElement)) return;
      syncIrFormStateFromElement(target);
    });

    $("loadAuditBtn").addEventListener("click", async () => {
      try { await loadAudit(); log("Audit list refreshed."); } catch (error) { log(`Audit load failed: ${error.message}`); }
    });

    $("clearAuditBtn").addEventListener("click", async () => {
      const source = ($("auditFilterSource").value || "").trim();
      const action = ($("auditFilterAction").value || "").trim();
      const scope = source || action
        ? `source='${source || "*"}', action='${action || "*"}'`
        : "ALL audit entries";

      if (!window.confirm(`Clear ${scope}? This cannot be undone.`)) {
        return;
      }

      try {
        const qs = new URLSearchParams();
        if (source) qs.set("source", source);
        if (action) qs.set("action", action);
        const path = qs.toString()
          ? `/api/v1/admin/audit?${qs.toString()}`
          : "/api/v1/admin/audit";
        const out = await api(path, { method: "DELETE" });
        log(`Audit cleared. Deleted ${out.deleted || 0} row(s).`);
        await Promise.all([loadAudit(), loadOverview()]);
      } catch (error) {
        log(`Clear audit failed: ${error.message}`);
      }
    });

    loadCollapsedCards();
    loadCollapsedDeviceCards();
    setupCollapsibleCards();
    restoreSession();
    setAuthState();

    if (state.user && state.refreshToken) {
      log(`Restored session for ${state.user.email}.`);
      connectRealtime();
      (async () => {
        try {
          await loadPreferences();
          await loadAll();
        } catch (error) {
          log(`Auto-load failed: ${error.message}`);
        }
      })();
    } else {
      log("Ready. Login with admin credentials to load live ops data.");
    }
  
