...
const envSchema = z.object({
  ...
  RELAY_COMMAND_TIMEOUT_MS: z.coerce.number().int().min(1000).max(30000).default(12000),
  RELAY_COMMAND_STATE_VERIFY_WINDOW_MS: z.coerce.number().int().min(0).max(10000).default(3000),
  RELAY_COMMAND_STATE_VERIFY_POLL_MS: z.coerce.number().int().min(50).max(2000).default(150),
  SECRET_MANAGER_FILE: z.string().optional(),
  OTA_SIGNING_PRIVATE_KEYS_JSON: z.string().optional(),
  OTA_PUBLIC_BASE_URL: z.string().url().optional(),
